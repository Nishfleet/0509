...
--
-- This SQL is the committed artifact. Regeneration is documented here rather
-- than scripted, because better-auth offers no flag-only way to emit plugin
-- tables — the CLI has to load a config that constructs the auth instance.
--
-- To regenerate, create this file temporarily and delete it afterwards:
--
--   // auth.cli.config.ts
...
--
-- Why SQLite and not D1: `auth generate` introspects a live database before
-- emitting — it is not a schema printer — and a D1Database binding exists only
-- inside workerd, so the CLI (a Node process) cannot reach one. It fails at
-- SqliteIntrospector.getTables with any placeholder. D1 is SQLite and the
-- emitted DDL is dialect-identical, so an empty in-memory SQLite database is
-- the correct stand-in, not a workaround.
--
