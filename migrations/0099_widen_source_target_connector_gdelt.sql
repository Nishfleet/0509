-- Widen source_target.connector_id CHECK to accept 'gdelt' (issue #3178,
-- mentions epic slice 2a: mainstream news via the GDELT DOC 2.1 API).
--
-- Same situation as migration 0093: SQLite cannot ALTER a CHECK constraint in
-- place, so the table is rebuilt with the repo's established table-rebuild
-- convention (0087's parent-with-children pattern, exactly as 0093 did).
--
...
-- see tests/integration/presence-migration-0093.integration.test.ts). The
-- three child row sets are therefore snapshotted into plain backup tables
-- first and restored after the rebuild, all inside the single transaction
-- D1 wraps the migration in.
--
-- Expand-only: every value the old CHECK accepted is still accepted, so
-- existing rows (including 'rss' rows from 0093) copy through unchanged, and
-- the running old code (which never writes 'gdelt') is unaffected. Rollback
-- of the PR removes code, never data.
...
CREATE TABLE source_target_gdelt_widen_new (
  id TEXT PRIMARY KEY NOT NULL,
  tracked_entity_id TEXT NOT NULL,
  user_id TEXT NOT NULL,
  connector_id TEXT NOT NULL CHECK (connector_id IN ('website', 'x', 'reddit', 'linkedin', 'rss', 'gdelt')),
...
INSERT INTO source_target_gdelt_widen_new (
...
ALTER TABLE source_target_gdelt_widen_new RENAME TO source_target;
