-- Teams incoming-webhook delivery: workspace/watchlist toggles and the
-- channel CHECKs on delivery_target + delivery_attempt are widened to
-- include 'teams' (mirrors the 0019 Slack channel migration shape).

ALTER TABLE workspace_delivery_config
  ADD COLUMN teams_enabled INTEGER NOT NULL DEFAULT 0;
