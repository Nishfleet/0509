...
--
-- 'bluesky' is likewise carried: the 0098 pair applies alphabetically
-- (bluesky, then gdelt), so the 0098_gdelt rebuild — the last of the two —
-- dropped 'bluesky' from the live CHECK, and the restore planner
-- (tests/d1-remote-restore-evidence.test.ts, catch-up case) also plans 0099
-- after 0098_bluesky, where the live CHECK carries 'bluesky' but not
-- 'gdelt'. 0099 therefore carries BOTH sibling values, which makes this
-- widen order-proof: the INSERT..SELECT copy satisfies the rebuilt CHECK
-- whichever way the 0098 pair applied.
...
CREATE TABLE source_target_threads_widen_new (
  ...
  connector_id TEXT NOT NULL CHECK (connector_id IN ('website', 'x', 'reddit', 'linkedin', 'rss', 'gdelt', 'bluesky', 'threads')),
