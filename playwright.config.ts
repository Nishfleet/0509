...
export default defineConfig({
  ...
  projects: [
    ...
    },
    {
      ...
      // Issue #1895 activation-scan spec is self-contained: it boots its own
      // isolated dev server (the shared webServer never sets
      // SIGNUP_FIRST_BRIEF_ENABLED, so the surface under test would be dark).
      name: "workspace",
      testDir: "./tests",
      testMatch: /(watchlist-run-history|activation-scan)\.spec\.ts/,
      timeout: 60_000,
      use: {
        ...devices["Desktop Chrome"],
        baseURL: localBaseURL,
