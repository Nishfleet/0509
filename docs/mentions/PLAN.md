...
| Honest coverage table ("is this source live for the customer?") | `app/lib/presence-source-coverage.server.ts` | Every source below gets exactly one entry; nothing ships as "live" that isn't. Since #3205 the catalog also renders publicly: /status ("Tracked sources") shows every source's posture, verbatim from this module — the coverage note states what the public surface covers, on the public surface. |
