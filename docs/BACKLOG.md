...
was once named as the source of truth here but was never created on disk
(verified 2026-08-25 in P7-C: `git log --all --diff-filter=AD` on the
path returns no commits in this repo, and the host path was never
written). The BACKLOG is the only ledger. Re-anchor this section after
every landed PR.
