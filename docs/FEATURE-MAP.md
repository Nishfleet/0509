...
| `/capture-plate` | `app/routes/capture-plate.tsx` | the URL. Nothing links to it yet | `Tab` to a plate, `Enter` | Three capture plates. The first is eager. The next two sit below the fold and are lazy. A stored pair opens in a dialog at 1440 and a sheet at 390. A missing object shows its reason and no image. | `e2e/capture-plate.spec.ts` |
| `/media/*` | `app/routes/media.$.ts` | the plate's `img` requests it | — | Serves one proof capture under `shot/e2e/capture-plate/` from the `0509-snapshots` bucket, resized through the Images binding using the `width` and `height` query. Any other key, including a missing one, answers 404 with the reason in text. | `e2e/capture-plate.spec.ts` |
