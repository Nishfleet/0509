...
|---|---|---|---|
| Toast layer | `app/components/toaster.tsx`, mounted once in `app/root.tsx`'s `Layout`; wired in `app/routes/settings.card.tsx` | a save lands — today the public-card publish/unpublish/rotate rows | sonner, styled from the tokens at radius 0. Only ever "saved" and "undo" (DESIGN.md §11): `toastSaved(message, undo)` is the one paved path, and the `undo` callback re-posts the inverse intent to the same action — not a second code path. The route's action reports `saved: "publish" \| "unpublish" \| "rotate"` in its data and the page raises the toast on the action result; rotate gets no undo because the released slug cannot be reclaimed. Any other toast call fails `tests/unit/toaster.test.ts`, which greps `app/` and `workers/` for a `sonner` import or a `toast(` call outside `app/components/toaster.tsx`, and `eslint.config.js` bans the import outside that module. The toaster section carries `aria-live="polite"` and sonner's stylesheet disables all toast motion under `prefers-reduced-motion`. Proof: `e2e/toast.spec.ts` does the edit on production at both viewports and runs axe on the live region. |
