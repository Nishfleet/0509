...
| `/login` | `app/routes/login.tsx` | the `/login` URL; nothing links to it yet | `Tab` to the email field, type, `Enter`; the passkey button is the next stop after submit | One centred screen, max 420px: the wordmark, one line, one email field, the button "Email me a link", and "use a passkey instead". Submitting POSTs to the same route; better-auth mints and sends a magic link, then the same screen becomes the sent state in place: the address, a 5-minute lifetime, and "send it again" disabled for 30 seconds with the count visible. It does not navigate, and it never reveals whether the address exists. The passkey button runs better-auth's authenticate ceremony (`generate-authenticate-options` → `verify-authentication`) and lands on `/app`, which redirects to `/onboarding` until the workspace has a self entity. | `e2e/login-sent.spec.ts` — sent state in place at 1440 and 390, resend disabled for 30 seconds with a visible count, no console errors, no horizontal scroll. The labelled input and passkey control are `e2e/smoke.spec.ts`. Sending is **J1** — `e2e/j1-magic-link.spec.ts`; the passkey ceremony is **J2** — `e2e/j2-passkey.spec.ts` |
