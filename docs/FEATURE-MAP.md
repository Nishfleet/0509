...
| `/app/settings/card` | `app/routes/settings.card.tsx` | the "Public card" link on `/app/settings` | `Tab` to a button, `Enter` | The public-card switch: turn it on (which mints an opaque URL), copy the link, rotate the URL, turn it off. Turning it off is what the public route's 404 is graded on — the edge copy expires on its own inside a minute, which is the contract's "404 within a minute". Each on/off save answers with a toast offering Undo — the inverse intent posted back through the same action; rotating the URL answers "New URL saved." with no undo. | `tests/integration/card/public-card.integration.test.ts` — the writer's publish, rotate, unpublish and two-workspace uniqueness against real D1 |
...
|---|---|---|---|
| Toast layer | `app/components/toaster.tsx`, mounted once in `app/root.tsx`'s `Layout` | an in-place save lands | sonner, styled from the tokens at radius 0. Only ever "saved" and "undo" (DESIGN.md §11): today the public-card on/off/rotate on `/app/settings/card`. Undo re-posts the inverse intent to the same route action — it is not a second code path. Any other toast call fails `tests/unit/toaster.test.ts`, which greps `app/` and `workers/` for a `sonner` import or a `toast(` call outside `app/components/toaster.tsx`. The toaster section carries `aria-live="polite"` and sonner's stylesheet disables all toast motion under `prefers-reduced-motion`. |
