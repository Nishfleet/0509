...
| fixture site | `workers/fixture-site.ts` (`workers/fixture-site.wrangler.jsonc`, Worker `0509-fixture-site`) | `GET https://fixture.0509.in/`; the break state is flipped with `POST https://fixture.0509.in/__break?mode=off\|hard\|soft` | The **J8** breakable own-site fixture (0509#4046): a separate Worker on `fixture.0509.in`, never on `0509.in` (a live 308 to `0509.io`). Serves a pricing-shaped page — one `h1`, a pricing section with real price tokens, a checkout link. The flip route is `POST`-only, token-guarded with constant-time `Bearer $FIXTURE_SITE_TOKEN`, and answers 503 when the secret is unset. `hard` answers 500; `soft` answers 200 with the pricing section gone, which is the case **P7.5**'s incident round-trip exercises. State is a KV key, so the whole break-fetch-repair-fetch cycle runs against production on demand. |
