...
|---|---|---|---|
| Toast layer | `app/components/toaster.tsx`, mounted once in `app/root.tsx`'s `Layout` | a save surface calls `toastSaved` — no route does yet; the mount and the lock ship ahead of the call sites | sonner, styled from the tokens at radius 0. Only ever "saved" and "undo" (DESIGN.md §11): `toastSaved(message, undo)` is the one paved path, and the `undo` callback is meant to re-post the inverse intent to the same action — not a second code path. Any other toast call fails `tests/unit/toaster.test.ts`, which greps `app/` and `workers/` for a `sonner` import or a `toast(` call outside `app/components/toaster.tsx`. The toaster section carries `aria-live="polite"` and sonner's stylesheet disables all toast motion under `prefers-reduced-motion`. |
