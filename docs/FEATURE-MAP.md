...
| `/robots.txt` | `app/routes/robots[.]txt.ts` | `GET /robots.txt` | — | The crawler policy, derived from the same manifest as the sitemap (`app/lib/public-routes.ts`, 0509#3989): `Allow: /`, `Disallow` `/app`, `/api` and `/mcp`, `Sitemap:` pointing at `/sitemap.xml`. No global `Disallow: /` on purpose — the landing's indexing is the page-level `robots: noindex` meta, and a global disallow would block the fetch that reads it. Also `Disallow` `/onboarding`, the signed-in first-run. `renderRobots` is unit-tested; the served file is the edge. | `e2e/smoke.spec.ts` — the served policy over HTTP; `tests/public-routes.test.ts` — the rules and the classifier agree |
| `/sitemap.xml` | `app/routes/sitemap[.]xml.ts` | `GET /sitemap.xml` | — | The search-engine index of public pages, derived from `app/lib/public-routes.ts` — the same manifest as `robots.txt`, so a surface cannot appear in one and be missing from the other. Lists `/login` today; `/` joins it the day the landing gate lifts, and `/pricing`, `/privacy`, `/terms` and each published `/s/<slug>` join when their routes do. The `/s/:slug` route exists (`app/routes/s.$slug.tsx`) but stays out of the sitemap until slugs can be enumerated from data (0509#3898). `application/xml`, `Cache-Control: public, max-age=300`. | `e2e/smoke.spec.ts` — the served urlset over HTTP; `tests/public-routes.test.ts` — every indexable manifest row served, schema-shaped, no duplicates |
