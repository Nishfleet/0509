...
| Route | File | Reach | Keyboard | What it does | Proof |
|---|---|---|---|---|---|
| `/onboarding` | `app/routes/onboarding.identity.tsx` | the first signed-in request, when the workspace has no self entity. A confirmed self entity sends this loader back to `/app`; an unconfirmed draft still shows the card. | the input is focused; `Tab` to it, type, `Enter` | One input — a domain, URL, `@handle` or channel URL — becomes the editable brand card, beside the signed-in email and the "Add a passkey" register ceremony (**J2**). The card builds as probes answer (each field says what will fill it, never a spinner); fields Jev is unsure about are outlined "check this". One action: "That's me", which logs edits as `user_decision` rows and starts `IdentityTailWorkflow` (public-subject gate → persist → seed watches → start discovery → first snapshot). | `e2e/onboarding-identity.spec.ts` — the session gate and the timed three-input journey (**J-series**); the passkey control is **J2** — `e2e/j2-passkey.spec.ts` |
