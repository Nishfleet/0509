...
| `/onboarding` | `app/routes/onboarding._index.tsx` | the first signed-in request, when the workspace has no self entity. `/app` redirects here, and a workspace that already has a self entity sends this loader back to `/app`. | the input is focused | Screen 1: one input, placeholder "your website, or a handle", and one action. Submitting POSTs to the same route and redirects to `/onboarding/identity`, carrying the raw input as `?input=` — the identity route owns normalising it (#3885), so this screen trims and passes through and never writes a second normaliser. An empty or whitespace-only submit renders one line with the input still focused, never an error page; the "nothing found anywhere" variant of that line is the identity engine's answer (all probes empty) and is asserted with the card in #3993. The mono step bar marks step 1. | `e2e/onboarding-one-input.spec.ts` — a real domain and a real `@handle` redirect, an empty submit's one line, no horizontal scroll at 390, no console errors |
| `/onboarding/identity` | `app/routes/onboarding.identity.tsx` | the redirect from `/onboarding` | — | Screen 2's landing point: the input arrives as `?input=`. The card that draws itself, and the "nothing found anywhere" line the identity engine answers with, are #3993. It carries the "Add a passkey" control (J2's register affordance) and the signed-in address, because screen 1 is nothing else but the one input (DESIGN.md §2.3). The step bar that marks step 2 is #4093's scope. | `e2e/onboarding-one-input.spec.ts` — the redirect target of a domain and a handle; **J2** — `e2e/j2-passkey.spec.ts` |
