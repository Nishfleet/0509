...
| `/design/brand-switch` | `app/routes/design.brand-switch.tsx` | the `/design/brand-switch` URL | `Tab` to the On switch, `Space` toggles it | Three states of the one per-brand switch: On with the consequence printed beside it, Off dimmed and dashed with "paused <date> · history kept", You on the green wash and not operable. No session. | `e2e/brand-switch.spec.ts` — three states, 44px hit area, no horizontal scroll, keyboard toggle, at 1440 and 390 in light and dark |
