...
|---|---|---|---|---|---|
| `/` | `public/index.html` | `https://0509.io` | — | The quiet rebuild notice. One `h1` (copy owned by `public/index.html`, not asserted verbatim). The footer links to privacy, terms, and the takedown address `support@0509.io` in plain text. Static file: no loader, no client state. `robots: noindex` until the gate lifts. | `e2e/smoke.spec.ts` — headline, contact link, no horizontal scroll at 390, no console errors. `e2e/footer.spec.ts` — the same footer on every rendered public page, at 1440 and 390. |
| `/login` | `app/routes/login.tsx` | the `/login` URL; nothing links to it yet | `Tab` to the email field, type, `Enter`; the passkey button is the next stop after submit; the footer is the last stop | One input plus a "Sign in with a passkey" button. Submitting the input POSTs to the same route; better-auth mints and sends a magic link, then the page swaps to "Check your email". Never reveals whether the address exists. The passkey button runs better-auth's authenticate ceremony (`generate-authenticate-options` → `verify-authentication`) and lands on `/app`, which redirects to `/onboarding` until the workspace has a self entity. The footer links to privacy, terms, and `support@0509.io`. | `e2e/smoke.spec.ts` — heading, labelled input, enabled buttons including the passkey control, footer address. `e2e/footer.spec.ts` — privacy, terms, and the takedown address. Sending is **J1** — `e2e/j1-magic-link.spec.ts`; the passkey ceremony is **J2** — `e2e/j2-passkey.spec.ts`; the never-reveals-existence claim is `e2e/magic-link-expiry.spec.ts` |
| `/privacy` | `app/routes/privacy.tsx` | the footer on every rendered public page | `Tab` to the home mark, then through the footer: privacy, terms, and `support@0509.io` | What we collect, how long we keep it, and how to be removed. Every sentence is backed by `docs/REBUILD-GUARDRAILS.md`. Says "one year", and does not state the thirty-day or ninety-day rules. The footer links to privacy, terms, and `support@0509.io`. | `e2e/privacy.spec.ts` — heading, who we refuse, what we collect, one-year retention including incident records, remove and forget, the 72-hour takedown row, the footer address, the wordmark loads `/`, no horizontal overflow, no console errors. The suite runs at 1440 and 390. |
| `/terms` | `app/routes/terms.tsx` | the footer on every rendered public page | `Tab` to the home mark, then through the footer: privacy, terms, and `support@0509.io` | What we will and will not do, and how to be removed. Every sentence is backed by `docs/REBUILD-GUARDRAILS.md`. Does not state the thirty-day or ninety-day rules. The footer links to privacy, terms, and `support@0509.io`. | `e2e/footer.spec.ts` — heading, who we refuse, degraded sources, robots.txt, the logged-out fetch, no paid data provider until approved, the 72-hour takedown row, no unbacked retention figure, no exclamation mark, and the shared footer. The suite runs at 1440 and 390. |
