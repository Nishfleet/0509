...
| `/privacy` | `app/routes/privacy.tsx` | the `/privacy` URL; nothing links to it yet | `Tab` to the home mark, then to `support@0509.io` in the footer | What we collect, how long we keep it, and how to be removed. Every sentence is backed by `docs/REBUILD-GUARDRAILS.md`. Says "one year", and does not state the thirty-day or ninety-day rules. The footer carries `support@0509.io`. Sits behind the same Cloudflare Access gate as `/login` until the audit lifts it. | `e2e/privacy.spec.ts` — heading, who we refuse, what we collect, one-year retention, remove and forget, the 72-hour takedown row, the footer address, no horizontal scroll, no console errors. The suite runs at 1440 and 390. |
