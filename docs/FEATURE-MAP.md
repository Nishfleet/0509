...
|---|---|---|---|---|---|
| `/` | `app/routes/landing.tsx` | `https://0509.io` | — | The quiet rebuild notice. One `h1` (copy owned by the landing route, not asserted verbatim), one contact link to `support@0509.io`. Static: no loader, no client state. `robots: noindex` until the gate lifts. | `e2e/smoke.spec.ts` — headline, contact link, no horizontal scroll at 390, no console errors |
