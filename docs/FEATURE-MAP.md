...
| `/api/auth/*` | `app/routes/api.auth.$.ts` | the browser follows the magic link here | — | better-auth's whole surface, mounted whole: magic-link request and verify, passkey registration and assertion, session, sign-out. Nothing is reimplemented above it. | **J1** — `e2e/j1-magic-link.spec.ts`; **J2** — `e2e/j2-passkey.spec.ts`; single-use replay, real-TTL expiry and request-path existence opacity — `e2e/magic-link-expiry.spec.ts` |
...
| `/robots.txt` | `app/routes/robots[.]txt.ts` | `GET /robots.txt` | — | The crawler policy, derived from the route registry (`app/routes.ts`) by `app/lib/public-routes.ts` (0509#3989): `Allow: /`, `Disallow` the protected trees `/app/`, `/api/`, `/mcp/` and the signed-in leaf `/onboarding`, `Sitemap:` pointing at `/sitemap.xml`. Rules are segment-scoped (`/app/`, not `/app`) so a public asset sharing a leading substring such as `/apple-touch-icon.png` is not over-blocked by RFC 9309 prefix matching. No global `Disallow: /` on purpose — the landing's indexing is the page-level `robots: noindex` meta, and a global disallow would block the fetch that reads it. The `Sitemap:` origin is `BETTER_AUTH_URL`, the one configured origin. `renderRobots` is unit-tested; the served file is the edge. | `e2e/smoke.spec.ts` — the served policy over HTTP; `tests/public-routes.test.ts` — the rules and the classifier agree |
| `/sitemap.xml` | `app/routes/sitemap[.]xml.ts` | `GET /sitemap.xml` | — | The search-engine index of public pages, derived from the same registry and manifest as `robots.txt`, so a surface cannot appear in one and be missing from the other. Lists `/login` today; `/` joins it the day the landing gate lifts, and `/pricing`, `/privacy`, `/terms` and each published `/s/<slug>` join when their routes do. The `/s/:slug` route exists (`app/routes/s.$slug.tsx`) but stays out of the sitemap until slugs can be enumerated from data (0509#3898). `application/xml`, `Cache-Control: public, max-age=300`. | `e2e/smoke.spec.ts` — the served urlset over HTTP; `tests/public-routes.test.ts` — every indexable manifest row served, schema-shaped, no duplicates |
