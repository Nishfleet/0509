...
| `/brand-switch` | `app/routes/brand-switch.tsx` | the `/brand-switch` URL; nothing links to it yet. `?theme=light` or `?theme=dark` sets the page theme. `robots: noindex`. | `Tab` reaches the first switch, `Space` toggles it. The You switch is visible and not in the tab order. | The one per-brand control, in its three states: On, Off, and You. The consequence sits beside the control before it is touched. Ranked rows do not host it yet — Home and Competitors are still stubs. | `e2e/brand-switch.spec.ts` |
