...
| `/terms` | `app/routes/terms.tsx` | the `/terms` URL; linked from the terms footer to `/privacy` and the support address | `Tab` through the two footer links, `Enter` | The subscription terms, written before the card goes down: three plans with monthly euro prices, no free plan, a 7-day trial with the card taken up front and the first charge on day 8 unless cancelled, Dodo as the biller, cancelling and refunds, what deleting a workspace deletes, and what the service reads (public data only, no completeness guarantee, degraded sources marked as degraded). | `e2e/terms.spec.ts` — 200, one `h1`, the trial and cancellation wording present, both footer links point at the right destination, no horizontal scroll at 390, no console errors |
