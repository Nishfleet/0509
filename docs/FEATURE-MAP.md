...
| `/brand-switch` | `app/routes/brand-switch.tsx` | the `/brand-switch` URL; nothing links to it yet. `?theme=light` or `?theme=dark` is read on the server and set on `<html data-theme>` before paint. `robots: noindex`. | `Tab` reaches the first switch, `Space` toggles it. The You switch is visible and not in the tab order. | The one per-brand control, in its three states: On, Off, and You. On prints the consequence before the control is touched. Off prints `paused <date> · history kept`. Ranked rows do not host it yet. Home and Competitors are still stubs, and preview-assert applies no session, so this page is where the keyboard proof runs. | `e2e/brand-switch.spec.ts` |
