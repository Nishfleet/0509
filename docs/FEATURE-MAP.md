...
| `/capture-plate` | `app/routes/capture-plate.tsx` | the URL only. Nothing links to it. Proof page, not a product destination | `Tab` to a plate, `Enter` | Three plates so the component can be opened at 1440 and 390. The first is eager. The next two sit below the fold and are lazy. A stored pair opens in a dialog on a wide screen and a sheet on a phone. A missing object shows its reason and no image. | `e2e/capture-plate.spec.ts` |
| `/media/*` | `app/routes/media.$.ts` | the plate's `img` | — | Proof reader for `shot/e2e/capture-plate/` only. Width and height are required. A missing or invalid dimension is 400 and the stored bytes are not returned. Any other key, including a missing object, is 404 with the reason in text. Resized through the Images binding. | `e2e/capture-plate.spec.ts` |
