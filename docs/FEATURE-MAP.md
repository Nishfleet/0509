...
| `/app/alerts` | `app/routes/app.alerts.tsx` | — | — | What changed. Shows undeliverable briefs (`alert.kind = 'delivery_failed'`, written by the `send-email-dlq` consumer) with the reason and the brief's text; other alert kinds are not listed yet. | **J7**, **J8** |
