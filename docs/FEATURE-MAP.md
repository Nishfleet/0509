...
| `/sitemap.xml` | `app/routes/sitemap[.]xml.ts` | `GET /sitemap.xml` | — | The search-engine index of public pages, derived from `app/lib/public-routes.ts` — the same manifest as `robots.txt`, so a surface cannot appear in one and be missing from the other. Lists `/login` today; `/` joins it the day the landing gate lifts and `/pricing`, `/privacy`, `/terms` and each published `/s/<slug>` join when their routes do. `application/xml`, `Cache-Control: public, max-age=300`. | `e2e/smoke.spec.ts` — the served urlset over HTTP; `tests/public-routes.test.ts` — every indexable manifest row served, schema-shaped, no duplicates |
