...
| `funnel_search_likely_confirm_signup_started` | Signup begins from a request whose URL carries the exact /search Likely-confirm marker (`source=search-likely-confirm`), recognized server-side against the allowlisted constant; the marker value itself is never stored | Measure whether the Likely-match row's one-click "Yes, that's them" confirm lifts anonymous search→signup conversion (issue 3306, BET 2 finish line) | `event_id`, `timestamp`, `route` | Email, name, the raw `source` query value, the searched/confirmed domain, referrer URL |
