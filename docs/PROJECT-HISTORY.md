...
- **Deploys go through CI, not your terminal.** Every push to `main` auto-deploys via `.github/workflows/deploy-production.yml` (the Cloudflare deploy token lives in the GitHub `production` environment). That workflow runs full tests → materializes private remote-restore evidence → `npm run deploy` → verifies the release evidence set. `npm run deploy` (`scripts/deploy-production.mjs`) type-checks the pinned tree ahead of `wrangler deploy` and executes the plan in `scripts/deploy-production-plan.mjs`: D1 migration-sync blocks; `launch:readiness:predeploy` (typecheck, tests, build, audit, and `e2e:local:release`) plus `verify-deploy-readiness.mjs` require a strict first-attempt Chromium `local-release` 66-entry Gate-B proof and block release; `cross_browser_risk_proof` (`scripts/run-cross-browser-risk-proof.mjs`) is a non-blocking diagnostic (`nonBlockingDiagnostic: true`); `remote_restore_evidence` (`scripts/verify-remote-restore-evidence.mjs`) blocks under policy `fresh-export-12h-reusable-schema-proof-7d` since 0509#3576: a backup **export record** published every 6 hours at the fixed R2 key `backups/d1/export-record.json` must be under 12 hours old, and the schema-fingerprint-matching **restore proof** is reusable for 7 days instead of 24 hours — so a full scratch restore (a real D1 row-import cost) is no longer paid on every deploy; post-deploy steps include rollback-target verification, launch-readiness canary cycle, version-bound release canary, Gate-C soak start, live public truth, and `e2e:prod:public` smoke. Do NOT also run `npm run deploy` locally for routine ships — on 2026-07-13 a local deploy interleaved with CI deploys, hashed asset names flip-flopped between versions, and live sessions (including the owner mid-sign-in) got unstyled pages and root error boundaries. Local deploy is break-glass only, when Actions is down and after checking no CI deploy run is in flight.
