...
- Every A issue re-queued: `discarded` removed, `agent-ready` added, body passes `lib/agent-ready-spec-gate.py check-body`.
  - 10/10 passed with rc=0 (re-verified from live bodies on this date).
...
- `lib/agent-ready-spec-gate.py check-body` over the 10 A bodies -> exit 0 each (10/10)
