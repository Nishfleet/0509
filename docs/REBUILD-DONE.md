...
Every number below is measured: the window was probed against production `0509`, and the write/restore/intactness numbers are from a drill actually run against a throwaway D1 on the same account. Nothing here is assumed from a document. The drill is issue #4179.

- **Recovery window: 30 days, and it is the platform's number, not ours.** On the production database `0509` (`746c6e3d-782e-443a-82d6-28ca93a16294`, storage `version: production`), `wrangler d1 time-travel info 0509 --timestamp=<unix>` resolved a 29-day-old timestamp to the bookmark `00005413-00000004-000050d1-bdb67f8fba23a8a31a761a98dd5bc68e`. A timestamp exactly 30 days old was refused outright, at every probe: `Invalid timestamp '1787501234'. Please provide a timestamp within the last 30 days`. So the boundary is sharp at 30 days, confirmed by probe rather than assumed. (Cost of a restore: `$0` additional per the D1 pricing page — that line is documentation, not a measurement; history and restores are not separately billed.)
...
- **Measured restore time: 7 s wall clock.** Unix start `1790093022` → end `1790093029`; UTC `2026-09-22T16:03:42Z` → `16:03:49Z`. The whole drill — take a bookmark, write two marker rows, restore, count all 31 tables before and after, delete the throwaway — took **442 s** end to end (`15:59:46Z` → `16:07:08Z`); most of that is 31-odd sequential verification queries, not the restore.
