...
| `docs/adstack-listing-2026-08-11.md` | ad-stack.ai (journal, no form) | ⛔ blocked | blocked: needs Nish's identity — one paste-ready email from `support@0509.io` to `hello@ad-stack.ai`; send stays NEEDS-NISH-gated (`scripts/send-vendor-mail.mjs` exists, dry-run default) | Send the prepared email (under a minute); targets the ~October 2026 quarterly re-test |
...
| `docs/alternativeto-listing-2026-08-11.md` | AlternativeTo (form) | ⛔ blocked | blocked: 2026-09-13 (issue #3393, two attempts recorded): plain fetch still 403 (Cloudflare, unchanged) and past it the signup form's hCaptcha returns no token to this datacenter VPS IP in a real Chromium (headless + headed tried, no silent pass, no solver/bypass) — the account cannot be created unattended | Create/verify the AlternativeTo account (a ready-to-reuse verification inbox — `fivetonine@uberip.com` — is recorded in the artefact doc), then a ~15-minute form fill; $5 priority review stays skipped (free backlog) |
