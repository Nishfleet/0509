...
- The footer address is `support@0509.io`. Nish or the deputy reads that mailbox by hand and decides by hand. No form, no ticket, and nothing writes the `takedown` row except that person.
- Measured 2026-09-22T15:34:00Z. Email Routing on `0509.io` is enabled. Rule `support-to-inbox` (`3340891ca5b04942aed88344f41a7ac8`) matches `to` = `support@0509.io`, enabled, priority 1. The action is Worker `0509-support-inbox`. A Worker action has no destination-address verified timestamp. The account has one verified destination address, on a different rule. This rule delivers to the Worker. The Worker has a mailbox object for `support@0509.io` and stores the message there. Nish or the deputy reads that store.
- A request is actionable when it names the subject's domain or handle, and who is asking. If either is missing, reply with this and write no row: "We need the subject's domain or handle, and who is asking, before we write a takedown row." Send that reply within 72 hours of the incomplete message. That reply does not start the removal clock.
- The 72 hours start when the actionable message arrives at `support@0509.io`. On that message, reply: "We have this request and will handle it by hand within 72 hours of its arrival at support@0509.io." When the row is written, reply: "We removed <subject> from tracking and from public standing cards on <date>." The row records the subject, the date, and the action.
- A request we do not honour asks for something other than that removal. Taking a subject off one workspace, or deleting someone else's account, is that case. Reply with: "We do not honour this request. We remove a named brand or person from our tracking and from our public standing cards, for every workspace, when the email names their domain or handle and who is asking."
