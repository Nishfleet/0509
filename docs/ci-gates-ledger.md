...
Tests standing beneath the fold: tests/required-context-no-skip.test.ts pins the folded unsharded required job and the new trusted-`github.sha` checkout shape (CodeQL #3069 remediation); assertion count unchanged — one assertion VALUE updated from the authorize-output ref to the trusted event commit, none removed.
>>>>>>> ae7e30362 (ci(gates): MERGE-IN batch 2 — one codex-node-checks context, path-gated folds, trusted-ref checkout (#3069))
