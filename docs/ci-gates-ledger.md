...
| ~~`ratchet-auto-tighten.yml`~~ | Design-system banned-marker drift | Background: monotone ratchet purpose | — | **REMOVED 2026-09-20** — 88/100 green, but nothing enforces the ceiling it maintains: `scripts/design-system-ratchet.mjs` is referenced by no PR check and no `package.json` script, so the weekly bot commit tightened a number no gate ever read. The actuator went first; `scripts/design-system-ratchet.mjs` and `verify-claims.mjs` followed the same day once the circular-canary sweep showed neither was reachable from CI. `docs/design-system-ratchet.json` stays as an editorial floor, read by `file-size-ratchet.test.ts`. |
