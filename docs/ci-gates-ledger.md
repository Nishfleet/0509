...
Tests standing beneath the fold: two tests pinned the required jobs' checkout shape — tests/required-context-no-skip.test.ts and tests/production-candidate-workflow.test.ts — both updated to pin the new trusted-`github.sha` ref for the two ci.yml required jobs (CodeQL #3069 remediation), per-row expected refs, every test and every expect() kept, none removed; tests/required-context-no-skip.test.ts also pins the `semgrep` bridge's merge_group-only exemption so the fail-closed queue adapter cannot drift.
