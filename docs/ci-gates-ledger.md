...
| batch 2 MERGE-IN #3069 (lands as #3350; #3222 closed — rebuilt per orchestrator review 2026-09-12: gate-owned attest/ratchet/auto-revert/CODEOWNERS rows split out to #3238) | before: ci.yml fired 5 PR jobs (codex-node-checks + shards 2/3/4 + dependabot-critical-check, release-proof excluded as merge_group-only); standalone `semgrep-actionlint` ungated on every PR (2 jobs), `backlog-console-refresh-test` (paths-gated), ungated `content-quality` Vale. Measured: PR-event hosted jobs (ops/count-pr-jobs.mjs, upper bound — event-conditioned steps unresolved) 16 → 10; all-workflow hosted jobs 44 → 38 by the same tool's pre-fix count; ci.yml PR wall-clock p50 19.7 min (30 most-recent successful PR runs, sharded era); merge-queue head wait 23 min at 2026-09-12T08:02Z (Nish reported 2h+ earlier the same day) | after: one `codex-node-checks` required context (shards folded into one unsharded `vitest run --project node` + coverage + `--project workers`) + `dependabot-critical-check` = 2 ci.yml PR jobs; semgrep+actionlint and backlog-console are path-gated STEPS inside codex-node-checks (`.github/**`, `automation/backlog-console/**`); `content-quality` path-gated to `docs/**`, `.vale/**`, `app/**/*.md`; `semgrep` keeps a merge_group-only bridge job because the ruleset still requires that context (deleted once admin drops it). Checkout in the required jobs pinned to trusted `github.sha` (CodeQL cache-poisoning remediation). After wall-clock p50 re-measured at merge | delta: ci.yml PR jobs 5 → 2; PR-event hosted jobs 16 → 10 (upper bound); workflow files 27 → 25 at the 2026-09-13 cut onto b1e76ca06 (#3345; main = 27 workflow files, verified) (26 → 24 at the 2026-09-12 4b9b25f7 cut; delta unchanged: semgrep-actionlint.yml, backlog-console-refresh-test.yml deleted); required contexts: shard-2/3/4 fold into codex-node-checks (ruleset update admin-side, below); all previously-required assertions still execute (unsharded suite runs every test the shards ran, plus workers + coverage on one runner). Advisory-coverage losses, acceptance-prescribed, no required context affected: semgrep+actionlint ran ungated on every PR and every push to main, now PR-side only on `.github/**` diffs and push-main coverage is gone (the merge_group bridge keeps full-tree queue scans, so merge-time protection holds); backlog-console hermetic test no longer runs on push to main (pre-fold state — the old workflow had no merge_group trigger either); Vale's push-main whole-tree lint now fires only on `docs/**`/`.vale/**`/`app/**/*.md` diffs |
...
1. Update the branch-protection / main-merge-queue ruleset required contexts to the folded names: remove `codex-node-checks-shard-2/3/4` (folded into `codex-node-checks`).
2. Delete the `semgrep` merge-queue bridge job in ci.yml once `semgrep` is out of the ruleset.
3. Post the admin `gate-integrity-attest: <head sha>` PR comment that lands this CI diff (it touches gate-owned paths, so this PR can never self-attest; the worker is forbidden from attesting).
...
Tests standing beneath the fold: two tests pinned the required jobs' checkout shape — tests/required-context-no-skip.test.ts and tests/production-candidate-workflow.test.ts — both updated to pin the new trusted-`github.sha` ref for the two ci.yml required jobs (CodeQL #3069 remediation), per-row expected refs, every test and every expect() kept, none removed; tests/required-context-no-skip.test.ts also pins the `semgrep` bridge's merge_group-only exemption so the fail-closed queue adapter cannot drift. The #3350 reviewer round added the fold-shape pins themselves — shard-2/3/4 stay folded, `pull_request` stays unfiltered, semgrep/actionlint/backlog-console stay path-gated steps, the bridge checks out trusted `github.sha` — and both Semgrep scan steps now fail closed when semgrep-results.json is missing or is not a JSON object.
