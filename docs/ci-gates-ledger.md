...
| ~~`backlog-console-refresh-test.yml`~~ | Regression in automation/backlog-console/refresh.sh | — | — | **REMOVED 2026-09-20** — with `automation/backlog-console/` itself. That console is fleet infrastructure, not 0509: it assembled fleet2 var state and published to nish.sh through `generate.py`, `push.sh` and the `fleet-console` Worker, none of which live in this repo. Its timer is installed on no host, its state directory `~/workspaces/agent-state/backlog-console` does not exist, and `Nishfleet/fleet-ops` does not carry a copy. A product repo was running CI for a dead fleet publisher. |
