...
- Magic-link emails open `/auth/better/magic-link`, a non-redeeming app confirmation URL. The URL carries the Better Auth token plus encrypted token-bound context and a non-guessable request state. The app requires the matching same-browser `f9_better_magic_state` cookie, moves the token into a short-lived HTTP-only confirmation cookie, and only then calls Better Auth's documented `magicLinkVerify` server API from a same-origin POST. The temporary state cookie is scoped to the parent `0509.io` domain so Safari users who start on `www.0509.io` and click an apex `0509.io` email link keep the same first-party state. Better Auth still owns token verification, expiry, single-use consumption, session cookie issuance, and the final redirect to `callbackURL` or `newUserCallbackURL`.
