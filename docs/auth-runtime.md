...
- Magic-link emails open `/auth/better/magic-link`, a non-redeeming app confirmation URL built from Better Auth's generated token URL plus a non-guessable request state. The app requires the matching same-browser `f9_better_magic_state` cookie, moves the Better Auth token into a short-lived HTTP-only confirmation cookie, renders a same-origin confirmation button, and only then calls Better Auth's documented `magicLinkVerify` server API from a same-origin POST. The temporary state cookie is scoped to `Path=/auth` so browsers can store it from `/auth/login` or `/auth/signup` and send it to `/auth/better/magic-link`; production custom domains also use `Domain=0509.io` so apex/www stay first-party. Better Auth still owns token verification, expiry, single-use consumption, session cookie issuance, and the final redirect to `callbackURL` or `newUserCallbackURL`.
