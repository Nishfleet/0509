...
- **The own-site lane is hourly and decides on ground truth only.** An `own-site-check` Workflow probes each customer's own homepage every hour with a plain `fetch`, never the browser: a bot wall or a thin page must not cost a browser session or raise an alarm. A 5xx, 404 or 410, or no answer from the apex and from `www.`, is probed again five minutes later, and a second broken answer opens the incident, pins the alert and sends the incident email. 401, 403, 429 and a Cloudflare challenge are "unknown" and never open anything. Each later hour is the promised re-check: a healthy probe closes the incident and sends the one "looks fixed" email. The lane does not depend on the `site.web` switch, so pausing the nightly sweep never silences outage alerts. Soft breaks (a 200 page that lost its pricing) are D3s, and wait for Jev.
- **One step never sinks the run.** A step that still fails after its retries is logged and counted, and the run moves on to the next page. A check step's snapshot id is fixed per run and page, and "previous" means before the run started, so a retry files the change instead of losing it.
- **Known limit, by design for now.** One instance walks every page in turn. At roughly 1 to 10 seconds a page, the nightly sweep has room for a few thousand pages, and so does the `plan` step's 1 MiB output. When the customer count gets near that, the scheduled instance should fan out one child instance per workspace.
