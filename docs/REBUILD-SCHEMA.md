...
**`source`** — the global registry. No workspace column by design: that is what makes a new source a row plus a plugin. Carries **`reliability`** (`official_api` / `rss` / `scraped_page` / `best_effort`) per the Jev contract.
