...
D1 migrations are forward-only. `wrangler d1 migrations apply` records each file by name in `d1_migrations` and never runs a recorded name again; there is no down command and no un-apply. Editing or deleting an already-recorded file changes nothing either — the name is in `d1_migrations`, so its contents are dead SQL. That is why the way back is always a new file, never an edit. Which file depends on what the wrong one did.

**Additive wrong** — the file added something that should not exist (a column, a table, an index). Write a new numbered migration that undoes it (`ALTER TABLE … DROP COLUMN`, `DROP TABLE`, `DROP INDEX` — and whatever index or CHECK the wrong file built over the added column). Nothing written before or since is lost, and the fix goes through ordinary review like any other change. Proven in `tests/integration/migration-rollback.test.ts`: the test applies the real chain to local D1, applies a deliberately wrong additive file through `applyD1Migrations` — the same `d1_migrations` filename record `wrangler d1 migrations apply` consults — then a corrective file, and asserts `sqlite_master` and the per-table column digest are identical before and after, with the wrong name recorded exactly once so a re-offer is a no-op.

**Destructive wrong** — the file dropped or rewrote a column that carried history. No forward migration can bring the rows back; they are gone at apply time. The only way back is Time Travel: `wrangler d1 time-travel restore 0509 --bookmark <bookmark>` (or `--timestamp` with a Unix-epoch or RFC3339 value, within the retention window — 30 days on Workers Paid, 7 on Free). A restore returns the **whole database** to that point — every table — so everything written since the bookmark is lost with it. That cost is why a destructive migration is a different class of change, not a bigger additive one.
