...
| AlternativeTo | **NEEDS_NISH_STEP** (not submitted) | Nish signs in (or creates and verifies) the AlternativeTo account, then the prepared suggestion can be submitted. The optional $5 priority review is skipped. Assets (squared icon + homepage and public-search screenshots) are committed under `docs/assets/alternativeto/` since 2026-08-12 — nothing else blocks submission. Re-verified 2026-09-13 (issue #3393, two attempts recorded): the worker path is exhausted — plain fetch 403 (Cloudflare, unchanged) and the signup form's hCaptcha yields no token to this datacenter VPS IP (real Chromium, headless + headed, no silent pass) — the owner step is unchanged. |
