...
Queried 2026-09-22 from Cloudflare's GraphQL Analytics API, account `f670a698e17bf160c8e4679823e68916`, window `datetime_geq` 2026-09-15T00:00:00Z and `datetime_lt` 2026-09-22T00:00:00Z. Seven UTC days. The raw responses, the query text, and the brand-count SQL are in the PR that added this section.

ON brands inside the window: 0. Production D1 `0509`, database `746c6e3d-782e-443a-82d6-28ca93a16294`, had one `entity` row with `state = 'on'` at query time. Domain `gymshark.com`, role `self`, `created_at` 2026-09-22T14:54:40.669Z, which is after the window. `snapshot` rows: 0. `watch` rows: 0. No line below is divided by a brand count. A per-brand-per-day figure needs a brand that was ON during the week.
...
| R2 `0509-landing-page-artifacts` | not a per-brand line in this model. The README calls this bucket an enhancement path | Class A 581: `PutObject` 548, `ListObjects` 30, `PutBucketLifecycleConfiguration` 3. Class B 1,972: `GetObject` 1650, `HeadBucket` 215, `GetBucketLifecycleConfiguration` 59, `HeadObject` 48. `DeleteObject` 65, free. Payload max 52,017,118,378 bytes plus metadata 75,591 bytes on 2026-09-21 | none. This bucket is not the snapshot engine | operations inside the included 1 million Class A and 10 million Class B. 52.02 GB decimal, if that one-day max had been the whole month, rounds to 53 GB, then 43 GB past the included 10, times $0.015, **$0.65**. One day's max is not a measured GB-month |
...
**100 brands.** This week has no per-brand rate, so it does not produce a 100-brand bill. The 10, 100 and 1,000 tables above stay estimates. The measured floor, script `0509` plus this D1 database plus the empty `0509-snapshots` bucket, repeating for a 30-day month, is the $5 subscription plus $2.31 of CPU, **$7.31**. That is under the $10 bar in `docs/REBUILD-DONE.md` §E. It is the bill at 0 tracked brands. It is not the bill at 100, and it is not a measured per-brand unit cost. §E's bar, under $10 at the current brand count with the per-brand cost measured, is not met yet. One brand exists, and it was created after this window.

The charter line, burn near the $5 base, is the same floor: $5 plus $2.31 of CPU. Adding the landing-page bucket's one-day storage max as if it lasted a month adds about $0.65 and still sits under $10. That bucket is not the per-brand engine.
