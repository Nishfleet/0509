...
> **Status 2026-09-20: this rollout is COMPLETE, and its validator is deleted.**
> `wrangler.jsonc` carries `MONITORING_FANOUT_MODE: "fanout"` and
> `MONITORING_FANOUT_GLOBAL: "1"` — the top of the ladder below. The per-rung
> validator (`scripts/monitoring-fanout-canary.mjs` plus its 383-line lib and
> *(guard test removed)*) was removed with the rest of
> `scripts/`: no workflow and no npm script CI ran ever invoked it, and once the
> last rung landed it had no job left.
>
> The document is kept as the record of how the migration was staged. Every
> command below is historical. If fan-out ever needs staging again, the rungs
> are config values in `wrangler.jsonc` and the state is readable from D1 —
> verify by reading those, not by rebuilding a validator whose own correctness
> nothing checked.
...
|-------------|-------------------|--------------|---------------|
| **Config** | *(guard test removed)* | *(validator removed — see the status note)* | `fanout` default; internal workspace secret set; `GLOBAL=1` |
...
1. **Shadow** on preview/staging — verify eligible counts only (*(validator removed — see the status note)*)
...
# validator removed 2026-09-20 — read the wrangler.jsonc vars instead
...
- If real scans complete well under the 30-minute cap in production, re-run the
  ...
  constant in the load-test harness (removed 2026-09-20; CI never ran it); the published numbers scale as
  `inflight * floor(2 * cadence / worst_run)`.

### Schedule-slip canary

*(validator removed — see the status note)*
