...
- Supabase is the retired legacy backend (the `legacy/` reference tree was removed from the working tree by #2991; it remains recoverable from git history); it is not part of the active `app/` or `workers/` runtime.
...
- The old `src/` Next.js app is no longer in the working tree (retired by #2991; recoverable from git history) and is no longer the live production runtime.
