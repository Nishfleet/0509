...
SELECT s.id, s.kind, s.aspect, s.published_at, s.observed_at, src.reliability,
       d3.p AS d3_p, d5.p AS d5_p, d6.p AS d6_p
FROM signal AS s
JOIN source AS src ON src.id = s.source_id
LEFT JOIN jev_verdict AS d3 ON d3.signal_id = s.id AND d3.question_id = 'noteworthy_change'
LEFT JOIN jev_verdict AS d5 ON d5.signal_id = s.id AND d5.question_id = 'mention_is_about_brand'
LEFT JOIN jev_verdict AS d6 ON d6.signal_id = s.id AND d6.question_id = 'mention_matters'
