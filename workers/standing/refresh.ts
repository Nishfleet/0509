...
import type { BucketCount, WeightRow } from "./score";
...
export async function refreshWorkspaceScores(
  ...
  const entityIds = (reads[0].results ?? []) as { id: string }[];
  const weights = weightsAsOf(
    (reads[1].results ?? []) as readonly WeightRow[],
    input.weekStartAt,
  );
  const counts = (reads[2].results ?? []) as readonly BucketCount[];
  const scores = scoreByEntity(counts, weights);
  const rows: StandingScore[] = entityIds.map((entity) => ({
    workspace_id: input.workspaceId,
    entity_id: entity.id,
    week_start_at: input.weekStartAt,
    score: scores.get(entity.id) ?? 0,
