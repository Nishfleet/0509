import { createCatchUp, type RolloverBinding } from "./schedule";
import {
  SIGNAL_COUNT_SQL,
  STANDING_UPSERT_SQL,
  WEIGHTS_AS_OF_SQL,
...
export async function refreshWorkspace(
  ...
  if (scores.length > 0) {
    ...
    await db.batch(
      scores.map((row) => db.prepare(STANDING_UPSERT_SQL).bind(
        crypto.randomUUID(),
        workspace.id,
        row.entityId,
        weekStart,
...
export async function freezeWeek(
  ...
  const frozen = freezeRanks(current.results.map((row) => ({
    entityId: row.entity_id,
    score: row.score,
    previousRank: previousRank.get(row.entity_id) ?? null,
  })));
  if (frozen.length === 0) return frozen;
  await db.batch(frozen.map((row) => db.prepare(
...
export async function runNightly(env: NightlyEnv, now = new Date()): Promise<string[]> {
  ...
  for (const workspace of listed.results) {
    ...
    ).bind(workspace.id).first<{ latest: string | null }>();
    const stale = needsCatchUp(latest?.latest ?? null, week);
    await refreshWorkspace(env.DB, workspace, now);
    if (!stale) continue;
    const id = await createCatchUp(env.STANDING_ROLLOVER, env.DB, workspace, workspace.standingInstanceId, now);
