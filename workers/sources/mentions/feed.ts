...
export function publisherHost(source: unknown): string | null {
  ...
  const urlValue =
    "url" in source ? source.url : "@_url" in source ? source["@_url"] : null;
...
export function viewsFromMedia(mediaGroup: unknown): number | null {
  ...
  const community =
    "media:community" in mediaGroup ? mediaGroup["media:community"] : null;
  if (!community || typeof community !== "object") return null;
  const statistics =
    "media:statistics" in community ? community["media:statistics"] : null;
