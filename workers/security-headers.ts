...
// injection, so Cloudflare inserts its RUM beacon script
// (https://static.cloudflareinsights.com/beacon.min.js) into every HTML
// response as it passes the edge. Without the host in script-src the beacon is
// blocked by the CSP and analytics silently records zero page views. The beacon
// posts to the same origin (/cdn-cgi/rum), which connect-src 'self' already
// permits — see the Cloudflare Web Analytics CSP guidance.
