...
---
# Deploy production restore-evidence gate (2026-08-11 lane 11) — already resolved by PR #545; residual failures fixed by PR #647

**Status: item already resolved on main; this lane records the evidence only.
No product code change made.**

Branch: `report/lane11-restore-evidence-gate-resolved`
...
The item's described cause is already fixed and merged on `main` by PR #545
(commit `555a9448`, "fix(deploy): generate exact restore evidence inline
instead of hard-failing"), and the residual deploy failures that continued
through 2026-08-11 were a different root cause — a Google Fonts hydration flake
that PR #647 (commit `389c0e55`, "fix(fonts): stop print→all stylesheet swap
from failing hydration") fixed on 2026-08-12. Both are ancestors of the
current `main` HEAD (`389c0e55`); no repository change is warranted from this
lane.

### 1. The pinned-SHA restore-evidence gate is fixed (PR #545, in `main`)

The prior lane (`report/lane6-restore-evidence-gate-already-resolved`,
commit `c70ceee6`) already recorded this fix on `main`; this lane re-verified
it against live 2026-08-11 deploy runs:

- `deploy-production.yml` carries the `generate_restore_evidence` job
  (fresh backup + isolated remote-restore drill at the exact pinned SHA on a
  fresh GitHub-hosted runner under the protected `production` environment)
  and the `cleanup_restore_evidence` job; the `deploy` job proceeds only when
  generated evidence passed the exact verifier or the fast path reused a
  verified artifact (`Generate` skipped).
- `scripts/ci-prepare-remote-restore-evidence.sh` reports
  `restore_evidence_available=false` (exit 0) for missing/stale/non-matching
  evidence instead of hard-failing, so the deploy generates its own exact
  evidence instead of depending on the separate nightly workflow.
- Direct-needs wiring fix (`edd99519`) is present: `prepare_remote_restore_evidence`
  sources its `backup_proof_status` output from its declared direct need
  `pin_candidate`; regression tests lock it
  (`tests/deploy-production-gate.test.ts`,
  `tests/production-candidate-workflow.test.ts`).

Live evidence (2026-08-11, runs against pinned SHAs): every failing run
passes `Authorize exact production candidate`, `Pin exact production
candidate`, `Prepare D1 remote restore evidence`, and `Generate D1 remote
restore evidence` (or skips it when pre-generated evidence is valid), then
dies in the unrelated `Deploy Worker` job. Run 31471216793's `Generate` job
ran the full fresh drill successfully (fresh backup + remote restore +
`d1-remote-restore-evidence.json` archive); runs 31530819590/31534318744
skipped `Generate` because valid pre-generated evidence existed. The
item's acceptance — a deploy of any `main` SHA proceeds without waiting for
the nightly drill — is satisfied.

### 2. The residual 2026-08-11 failures were a hydration flake, fixed by PR #647

14 consecutive `Deploy production` runs on 2026-08-11 (e.g. 31471216793,
31499017404, 31504008180, 31509456319, 31514742997, 31520394937,
31525492418, 31530819590) failed at the `Deploy` step inside the
`Deploy Worker` job with:

```
launch:readiness:predeploy failed — manifest status=failed;
strictIssues=["browser_hydration_error:console","browser_run_failed"]
```

This is not the restore-evidence gate: it is a `reactHydrationError`
annotation with source `console` surfaced by
`scripts/playwright-release-manifest-reporter.mjs` during the release
readiness e2e. Root cause was the non-blocking Google Fonts pattern flipping
`media="print"` to `"all"` in an inline script before React hydrated when
css2 was already cached, tripping `browser_hydration_error:console` even
though all Gate-B journeys passed.

PR #647 / commit `389c0e55` (on `main` 2026-08-12, the current `main` HEAD)
suppresses that intentional mismatch on the stylesheet link and applies the
swap immediately when `link.sheet` is already set, with regression test
`tests/root-fonts-async.test.ts`. The first deploy dispatch at that SHA
(run 31534318744) **completed successfully** (2026-08-12): it passed the
evidence gate (`Generate` skipped, valid pre-generated evidence), passed
Typecheck/Test/Materialize evidence/Canary sync, and finished the full
deploy + Gate C release sequence — the first green `Deploy production` run
since the billing block cleared, confirming both the evidence gate and the
hydration fix end to end.

Other intermittent failures observed in the same window are unrelated to
this item and already addressed on `main`: `Synchronize private canary token`
failed with "latest version of your Worker isn't currently deployed" when a
previous deploy had not shipped (run 31525492418; canary sync reordered
post-deploy by `9dd46a65`), and one `remote_main_drift` CAS death (run
31471216793) was fixed by #630 (`f5aabf14`).

## Evidence on current main

- `git merge-base --is-ancestor 555a9448 origin/main` → true (PR #545 in main).
- `git merge-base --is-ancestor 389c0e55 origin/main` → true (PR #647 in main;
  it is the current `main` HEAD).
- Live run 31471216793: `Generate D1 remote restore evidence` ran the fresh
  drill and succeeded; `Confirm scratch D1 cleanup` succeeded; `Deploy Worker`
  was the only failing job.
- Live runs 31530819590 / 31534318744: `Generate D1 remote restore evidence`
  skipped (valid pre-generated evidence found); `Prepare D1 remote restore
  evidence` succeeded; the gate is green. Run 31534318744 (at `389c0e55`,
  the PR #647 fix) completed **success** end to end on 2026-08-12.
- Failed `Deploy` steps across 8+ runs all report the same
  `browser_hydration_error:console`/`browser_run_failed` manifest status —
  no run in the window failed on `remote_restore_*` evidence issues.
