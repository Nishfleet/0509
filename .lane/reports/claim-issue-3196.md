...
- `/ads/:domain` LinkedIn Ads section: the honest coverage note (source — the public LinkedIn Ad Library, no official-API key that bars commercial use; ad types — promoted-post cards, no format distinction; what is NOT covered — spend, reach, audience metrics; region — the United States, the public search's verified geo=US posture, no other regions; freshness — the weekly cadence, "Last checked" = the last successful capture) via the shared brand-page `source-sections.tsx`. Surface truth (senior-review correction, no longer claimed wider): the note renders on /ads and the docs copy — the acceptance's two note surfaces. The watchlist competitor-detail (timelines) shows the LinkedIn ads through its own #2193-era section (`components/sources/linkedin-ads.tsx`, its own note); briefs render no source sections at all. Widening the #3196 note to the competitor-detail section is queued as a follow-up issue, not this slice. The section now renders the #2193 stored payload's REAL fields (promoted text, advertiser, public detail link) instead of the TikTok-shaped adId/firstSeen ghost reads.
