...
- [Public search cold path (2026-08-12 lane 12)](#public-search-cold-path-2026-08-12-lane-12--already-resolved-by-prs-543544547559) — already resolved; evidence record only
...
---
# Public search cold path (2026-08-12 lane 12) — already resolved by PRs #543/#544/#547/#559

**Status: already resolved; this lane records the evidence only.**

Branch: `report/lane12-public-search-cold-path-already-resolved`
Base: `origin/main` at `389c0e55`

## Item

- [ ] Public search cold path: first anonymous query for an uncached advertiser
  takes ~20s before any useful result or p[age]

## Verdict

No code change was warranted. The first anonymous query for an uncached
advertiser already returns an honest in-progress ("warming") page immediately
(well under a second of provider work), while the ~20s browser capture finishes
in the background via `waitUntil`; the client's existing 5s x 12 warming poll
swaps in the finished capture without any request hanging open. The resolving
commits are all ancestors of the current `main` HEAD (`389c0e55`), and no later
commit touches the cold-path logic (`app/lib/ad-source.server.ts`,
`app/lib/search-execution.server.ts`, `app/lib/meta-library-browser.server.ts`,
`app/routes/search.tsx`):

- PR #543 / `8b3e1e30` — the first anonymous query returns the typed warming
  state immediately and runs the ~20s browser capture in the background via
  `waitUntil` when this request owns the distributed discovery lease
  (`wantsBackgroundWarm` + short lease + heartbeat self-heal).
- PR #544 / `b5bf3ce7` — an expired-but-usable cache entry paints immediately
  (labeled `cache_only` with the warming flag) instead of rerunning the capture
  synchronously after public-search TTL expiry.
- PR #547 / `5b053b14` — public-search lease waiters return the typed warming
  state immediately instead of burning the 12s wait budget.
- PR #559 / `90147b9b` — the anonymous submit stays on "Searching…" while the
  cold-path search resolves, and releases with the same 60s budget as the
  warming poll.
- Honesty follow-ups in the same area: PR #567 / `5e682868` (public /search
  "right now" promise gated on a proven fresh-live capture), PR #612 /
  `90cea3a5` (honest end state when the anonymous check outlives the 60s
  warming cap), PR #579 / `1d00f084` (honest anonymous form error/status
  states), PR #616 / `9f0805d0` (anonymous search no longer commits visitor geo
  country).

## Why the item was re-assigned

Lane 8 (2026-08-10) reached the same verdict and recorded it, but its evidence
PR #596 (`report/lane8-public-search-cold-path-already-resolved`, commit
`317c552c`) was closed without merging, so the item never closed in the tracker
and was re-assigned as lane 12. This record supersedes it, re-verified on the
current tip.

## Evidence on current main

- **Server cold path** (`app/lib/ad-source.server.ts` →
  `searchAdsViaSourceResolver`): when the request owns the distributed
  discovery lease and qualifies (meta_library_browser provider, public_search
  route, no UNEXPIRED entry, no cursor, request ExecutionContext present), the
  capture runs via `waitUntil` and the loader returns `discoveryProgress:
  "warming"` with `ads: []` immediately. Requests that do NOT own the lease
  (the concurrent visitor or the polling revalidation while a background
  capture runs) get the same typed warming state without blocking, because the
  public-search waiter returns immediately instead of waiting out its budget.
  Expired-but-usable entries render immediately labeled `cache_only` + warming.
  The cold-path background capture holds a short (60s) lease renewed by a 20s
  heartbeat, so a capture canceled by the waitUntil 30s cap self-heals: the
  lease expires and the next poll re-acquires and runs discovery inline.
- **Client poll** (`app/routes/search.tsx`): while `discoveryProgress ===
  "warming"`, the page auto-revalidates every 5s (12 polls, 60s cap), resets
  the budget when the query identity changes or a committed retry lands, and
  after the cap renders an honest end state with an explicit retry instead of a
  contradictory terminal answer. The submit button stays pending only while
  the cold-path search resolves.
...
- `tests/ad-source.test.ts` — "returns the warming state immediately and
  ...
  "returns an expired-but-usable entry immediately and refreshes it in the
  background when this request owns the lease", "returns an expired-but-usable
  entry with the warming flag while another isolate owns the live search",
  "returns a typed warming state immediately while another isolate owns the
  live search", "keeps cursor (load-more) requests on the synchronous discovery
  path", "keeps forceLive public searches on the synchronous discovery path",
  plus lease-TTL/heartbeat coverage.
- `tests/search.route.test.ts` ("renders the first-request cold-path warming
  summary as an honest in-progress line") and
  `tests/search-warming-state.test.ts` (submit stays on "Searching…" while
  warming; honest terminal state with a working retry after the poll budget).

## Verification on this tip (origin/main `389c0e55`)

- `tests/ad-source.test.ts` + `tests/search-warming-state.test.ts`: 2 files,
  85/85 passed.
- `tests/search.route.test.ts` + `tests/search-execution.test.ts` +
  `tests/search-v2.test.ts` + `tests/e2e-search.server.test.ts` +
  `tests/search-submission-settle.test.tsx`: 5 files, 76/76 passed.
- `tests/search-selection.paint-fast.test.ts` +
  `tests/search-load-more.test.ts` + `tests/search-live-claim.test.tsx`: 3
  files, 21/21 passed.
- `npm run typecheck`: exit 0.
...
Pull request: https://github.com/Nishfleet/0509/pull/586 (opened by this lane)
