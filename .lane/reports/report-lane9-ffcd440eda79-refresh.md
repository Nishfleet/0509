...
---
# Redirecting internal links on home — dogfood ffcd440eda79 (2026-08-12 lane 9) — fix in flight on PR #570, refreshed against main

**Status: fix in flight on PR #570; this lane refreshed the PR branch against
current `origin/main` and re-verified it end to end. Acceptance rerun belongs
to the loop after merge + deploy (the SEO Fix Kit engine audits the live site,
which still serves the old links until PR #570 lands).**
...
The item is already covered by PR #570 (`fix/lane1-home-internal-link-redirects`,
commit `08f05eac` → head `b354a7c0`), which was CI-green but had drifted
BEHIND `origin/main`. Per the improvement-loop convention for items with an
in-flight fix PR ("do not open a duplicate PR" — same convention lane 16
recorded for this item), this lane makes no duplicate product change. Instead
it brought the PR branch up to date and re-verified the fix on the merged tip:

- Merged current `origin/main` (`389c0e55`, four commits: #647 fonts hydration,
  #638 Meta ads docs, #625 search proof-capture label, #630 CAS deploy sync)
  into the PR branch → new head `dc726b24`, pushed to
  `fix/lane1-home-internal-link-redirects` on 2026-08-11T22:07Z. PR #570
  moved from BEHIND to MERGEABLE/BLOCKED (BLOCKED = required checks
  re-running on the new head).
- The only diff introduced is main's own commits; no product change by this
  lane.

## Verification on the merged tip (`dc726b24`)

- `npm run typecheck` (the repo's real gate) passes.
- Full test suite: **428 files / 4896 tests pass** (`npm test`).
- Focused route/component tests for every touched surface pass:
  `tests/app-link.test.ts`, `tests/marketing-nav.test.ts`,
  `tests/help-runtime-truth.test.ts`, `tests/public-doc-routes.test.ts`,
  `tests/status.route.test.ts`, `tests/ads-brand-page.render.test.tsx`,
  `tests/changelog-customer-value.test.ts`,
  `tests/compare-magicbrief.route.test.ts`,
  `tests/compare-meta-ad-library.route.test.ts` — 43 tests, all green.

## Evidence on current main (before merge, still accurate)

- Anonymous-visible `/app*` links on the four public pages are exactly the
  ones PR #570 rewires via the shared `appLinkTarget` helper
  (`app/lib/app-link.ts`): MarketingNav "Open app" (`app/components/marketing-nav.tsx`)
  and the in-content links in `app/routes/help.tsx` (`/app/notifications`,
  `/app/support?category=delivery`, `/app/billing`,
  `/app/support?category=billing`, `/app/support?category=security`,
  `/app/support`), `app/routes/docs.tsx` and `app/routes/api.docs.tsx`
  (`/app/developer-access`).
- Home's remaining direct link (`/app/billing?source=top-up#top-ups`,
  "Manage packs" in `app/routes/marketing.tsx`) is session-gated
  (`rootData.session && hasBundlePrice(...)`), so anonymous crawlers never
  see it — no redirect, nothing to change.
- `appLinkTarget` output is byte-identical to the app-route guard's redirect
  target (`requireSession` in `app/lib/auth.server.ts`):
  `/auth/login?redirectTo=<encoded path>` — the same final URL minus the
  redirect hop, matching the engine's own fix guidance ("Update internal
  links so they point directly to the final destination URL").
...
The finding reproduced on 2026-08-09 before the PR; a production rerun after
merge+deploy is the loop's close-out.

## Files

- `.lane/report.md` — evidence record only; no product code touched.
- `fix/lane1-home-internal-link-redirects` (PR #570) — refreshed to
  `dc726b24` (main merge), no product changes by this lane.
