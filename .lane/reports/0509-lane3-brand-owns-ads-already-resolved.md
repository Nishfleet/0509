...
#561, and re-verified at the current tip `422fbd55` (#806). No product code
touched by this lane.**

Branch: `0509-lane3-brand-owns-ads-already-resolved`
...
The item is **already landed on `origin/main`**. The resolving commits are
ancestors of the current main tip, and this lane's re-verification at that tip
...
A prior lane (lane 1, 2026-08-20) already recorded evidence for the same item
at `ab7b5f96` (#723). This lane re-verifies at the newer tip `422fbd55`
(#806); the only commits touching the brand-page surfaces since that
verification are further hardening (verified-link-evidence gate #619, live
claim moments-ago boundary #652), never a regression.
...
- `tests/ads-brand-page.render.test.tsx` — 20 tests: every section of the
  populated page renders in briefed order, including none-owned/mixed/
  unconfirmed surfaces and JSON-LD attribution.
- `tests/ads-brand-page.route.test.ts` — 34 tests: loader counts and meta
  attribution.
...
- `.lane/reports/0509-lane3-brand-owns-ads-already-resolved.md` — this
