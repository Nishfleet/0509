...
- LinkedIn: the public-library search (the builder at
  linkedin-ad-library.server.ts:169) sets `geo: "United States"` → copy says
...
- `npx vitest run --configLoader runner --project node --changed origin/main`
  (branch rebased onto current origin/main 522c79d4c first) → 49 files,
  572/572 passed, exit 0, FIRST RUN clean (inner loop 1/5 rounds, nothing
  to adjudicate).
...
- **Gitleaks incident (live, not vibes):** required check `Gitleaks` = FAIL,
  ...
  Root cause: a 14-char backticked identifier in this report's parity
  section matched gitleaks' 14-alphanumeric linkedin-client-id heuristic —
  entropy 3.52, a prose backtick, not a credential. Same false-positive
  class as the two precedents already in `.gitleaksignore`.
- **Fix:** the documented house convention — fingerprint + why-comment
  ...
  product code, no workflow, no mechanism touched. (2026-09-17 resume:
  that hunk now lives only on the closed PR #3401's chain — banked in the
  closed PR's head ref and the pre-rebuild backup branch — because this
  fresh chain rewords the prose instead, so the false positive never
  exists here and no gate-owned path is touched.)
