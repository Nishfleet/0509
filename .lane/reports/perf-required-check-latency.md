...
|---|---|---|
| **A: Dependency + vitest + tsc cache** | `actions/setup-node` cache is already configured for `package-lock.json`. Add `actions/cache@v4` for `node_modules/.vite/` (vitest 4.x transform cache) and `tsconfig.*.tsbuildinfo` (incremental `tsc -b`). Typecheck is 25 % of wall-clock and dominated by `tsc -b`; its `.tsbuildinfo` cache survives today only in the runner, which is wiped between runs. | **Apply, but only on `pull_request`.** CodeQL flags `actions/cache` after an untrusted checkout in any trigger that can write to the default branch cache scope. Restricting the steps to `pull_request` keeps the PR-branch cache while avoiding the alert. Expected 5–15 s on typecheck + modest Test-step improvement on warm cache. |
...
1. `.github/workflows/ci.yml` — add `actions/cache@v4` for `node_modules/.vite/` and `tsconfig.*.tsbuildinfo` only on `pull_request` (guarded by `github.event_name == 'pull_request'`), then shard the existing `codex-node-checks` job and add 3 sibling shard jobs (each is a real required-context job). Existing job contract preserved (no `if:` on the job, no `needs:`, no `continue-on-error`, no trailing `|| true`; authorizer remains step 1; pinned `actions/checkout@3d3c42e…`; verify-authorized-checkout step preserved).
...
- `.github/workflows/ci.yml` (add caches; shard `codex-node-checks` into 4 jobs).
