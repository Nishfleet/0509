...
1. `.github/workflows/ci.yml` — improve cache + shard the existing `codex-node-checks` job + add 3 sibling shard jobs (each is a real required-context job). Existing job contract preserved (no `if:`, no `needs:`, no `continue-on-error`, no trailing `|| true`; authorizer remains step 1; pinned `actions/checkout@3d3c42e…`; verify-authorized-checkout step preserved).
