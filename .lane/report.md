...
---
# Public /search "Landing-page signal" count (2026-08-10 lane 2) — already resolved by PR #553

**Status: already resolved; this lane records the evidence only.**

Branch: `report/lane2-landing-signal-count-already-resolved`
...
landing-page signals", merged 2026-08-09 (07:40:48Z), an ancestor of the
current `main` HEAD (`d109e2d2`). No open PR duplicates the item.
...
- **Count source**: `buildSearchAnswer()` in `app/lib/search-answer.ts` is the
  ...
  caller is the public search route (`app/routes/search.tsx`, line 1264).
  Current main counts only captured snapshots:
...
- **No other public-search surface overcounts**: `app/lib/search-display.ts`
  ...
  captured signals. The workspace-collections insight (`app/lib/insight-depth.ts`)
  labels URL-only honestly as "Destination tracked", not as a captured signal.

## Verification on this tip (origin/main `d109e2d2`)

- `vitest run tests/search-answer.test.ts`: 1 file, **18/18 passed**,
  including both PR #553 regression pins.
- Full suite `npm test`: **423 files / 4848 tests passed** (same HEAD as the
  sibling lane-2 report, which ran the full suite on `d109e2d2`).
