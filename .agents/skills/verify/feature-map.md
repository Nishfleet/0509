...
lie. `app/routes.ts` is the only registry — start there, not from a guess. It
lives inside the `verify` skill: `../SKILL.md` drives the app over the Chrome
DevTools protocol and this map is what tells it which rows to drive.
