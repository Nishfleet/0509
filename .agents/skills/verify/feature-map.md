...
| `/app/competitors` | `app/routes/app.competitors.tsx` | —; keyboard: — | The tracked set. Currently a stub. | **J6** |
...
| `/app/alerts` | `app/routes/app.alerts.tsx` | —; keyboard: — | What changed. Currently a stub. | **J7**, **J8** |
| `/app/settings` | `app/routes/app.settings.tsx` | —; keyboard: — | Workspace settings. Links to the public card. | **J13**, **J14** |
...
These six surfaces have no user route: no URL maps to them and no e2e spec opens them, so their **Reach** cell says what reaches them instead — a system, cron or inbound-HTTP entry point. They are listed here so an agent driving the app does not conclude they must already exist somewhere it has not looked.
