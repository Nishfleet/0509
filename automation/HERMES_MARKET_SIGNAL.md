...
3. The daily D1 snapshot is generated by the repository's scheduled `market-signal-snapshot.yml` GitHub Actions workflow, which authenticates to Cloudflare with repository token secrets instead of a host wrangler OAuth session (that session expired 2026-08-04T13:26:24Z and cannot refresh in a non-interactive cron; see PR #557). The snapshot no longer lands on `main`: the Nishfleet org forbids Actions from creating PRs, so the workflow pushes the snapshot to the dedicated data branch `automation/market-signal-snapshot` instead (no PR, no policy weakened). Fetch that ref and read the snapshot with `git fetch origin automation/market-signal-snapshot && git show origin/automation/market-signal-snapshot:ops/market-signal/0509-market-signal.json` into a temp file, then parse the temp file. Do not run `wrangler` on this host; do not rely on a host-generated snapshot.
