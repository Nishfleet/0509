...
0. **Pre-flight workdir blocker check.** Before fetching or touching anything, run `git -C <workdir> status --porcelain` and stop if it prints anything (`scripts/check-market-signal-workdir.sh`). Exit 0 = safe to continue. A non-zero exit means an untracked file on disk sits at a path `origin/main` also has, so git would refuse the fast-forward ("untracked working tree files would be overwritten by merge") — the exact blocker that failed the 2026-09-08 morning run via an un-saved `tests/ad-aggression-page.render.test.tsx` (issue #1953). Do NOT fetch, fast-forward, or publish when it fails: fail loud with the exact paths the script prints and stop. The previous report stays untouched.
...
Write both reports and the exact Telegram message as candidate files. The raw candidate must use a new UTC timestamp. Check the report has the required front-matter and sections (`grep -c` the headings listed below) using today's Asia/Kolkata date. It must print `market_signal_report_valid` before publication.
