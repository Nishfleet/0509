...
0. **Pre-flight workdir blocker check.** Before fetching or touching anything, take the fast-forward with git itself and let it refuse:

    ```bash
    git fetch origin main && git merge --ff-only origin/main
    ```

    Git fails with `untracked working tree files would be overwritten by merge` and names every offending path — the exact blocker that failed the 2026-09-08 morning run via an un-saved `tests/ad-aggression-page.render.test.tsx` (issue #1953). Do NOT publish when it fails: read the paths git printed and stop. The previous report stays untouched.

    (`scripts/check-market-signal-workdir.sh`, 87 lines plus a 195-line test, reimplemented that detection so it could be run *before* the fetch. It was deleted 2026-09-20: it was invoked by no workflow, and `--ff-only` produces the same list with the same exit code from the command you were going to run anyway.)
