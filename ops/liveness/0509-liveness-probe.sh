...
probe_shallow() {
  ...
  if ! SHALLOW_PAYLOAD="${SHALLOW_PAYLOAD}" python3 - <<'PY'
import json
import os
import re
...
if identity.get("searchRolloutMode") != "v2":
...
probe_deep() {
  ...
  if ! DEEP_HEALTH_PAYLOAD="${DEEP_HEALTH_PAYLOAD}" DEEP_EXPECTED_WORKER_VERSION="${DEEP_EXPECTED_WORKER_VERSION}" python3 - <<'PY'
import json
import os
