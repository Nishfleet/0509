...
if identity.get("searchRolloutMode") != "shadow":
    raise SystemExit("health search rollout mode was not shadow")
PY
  then
...
if identity.get("searchRolloutMode") != "shadow":
    raise SystemExit("deep health search rollout mode was not shadow")
PY
  then
