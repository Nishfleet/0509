...
- `npm test` runs WITHOUT coverage on purpose (fleet-ops#4891): coverage forks cost ~0.8 GB each and `tsc -b` ~2 GB, which on the 16 GB fleet box priced every worker at 2 GB and capped intake at 2-3 workers. CI (`ci.yml`) still runs `--coverage` and `npm run typecheck`. Autonomous workers: run the node project scoped to touched test files (`npx vitest run --configLoader runner --project node <files>`), run the `workers` project only when `migrations/**` or `tests/integration/**` changed, and do not run `npm run typecheck` locally — CI owns it. Use `npm run test:coverage` only when you need the lcov report.
