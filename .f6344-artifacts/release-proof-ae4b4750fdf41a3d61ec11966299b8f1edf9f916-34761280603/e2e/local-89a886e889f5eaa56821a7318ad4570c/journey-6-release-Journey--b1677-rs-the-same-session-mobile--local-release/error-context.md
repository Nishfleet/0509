...
- Name: journey-6-release.spec.ts >> Journey 6 release: recovery across account, support, retention, auth, and team >> shows a safe auth outage and recovers the same session (mobile)
- Location: e2e/journey-6-release.spec.ts:316:5
...
```yaml
- alert [active] [ref=f1e3]:
  - heading "Temporarily unavailable" [level=2] [ref=f1e4]
  - paragraph [ref=f1e5]: Authentication is temporarily unavailable. Please try again in a moment.
  - generic [ref=f1e6]:
    - button "Try again" [ref=f1e7]
    - link "Contact support" [ref=f1e8] [cursor=pointer]:
      - /url: /app/support
```
