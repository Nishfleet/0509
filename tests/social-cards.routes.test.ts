/**
 * Route-level og:image coverage split out of tests/social-cards.test.ts so
 * both files stay under the tests/ file-size ratchet (issue #2376) while
 * landing issue #3414's .png flip -- the split is the ratchet's prescribed
 * remedy, so no seed-list edit. Moved as-is (no test changed): the
 * every-buyer-surface og:image stamps, the /ads and /timeline route meta
 * stamps, the sitemap-cohort og:image:type agreement (issue #2089), and the
 * malformed percent-encoding parser hardening (issue #2465). The test
 * environment (react-router mock, helpers) is identical to the original
 * file's, so every moved test behaves exactly as before.
 */
...
import { canonicalUrl, STATIC_SURFACE_SOCIAL_CARDS } from "~/lib/seo";
