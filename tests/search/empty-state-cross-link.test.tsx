...
describe("search empty-state next action (issue 3021)", () => {
	it("compresses to one honest line plus one primary action", async () => {
		...
		// The honest "not evidence" sentence is told exactly once in
		// visible text — the answer panel's near-doc note and the summary
		// sub-line are suppressed. A single sr-only aria-live announcement
		// also carries the sentence for screen readers and is not a second
		// visible telling.
		const visibleMarkup = markup.replace(
			/<div[^>]*aria-live="polite"[^>]*>[\s\S]*?<\/div>/g,
			"",
		);
		expect(
			visibleMarkup.match(/not evidence that the competitor is inactive/g),
		).toHaveLength(1);
		...
		// handoff — one real next step replaces the reading list. (The page
		// keeps /capture-rules on the signup trust-proof note further down,
		// issue 2049 — that is a different surface.)
		const emptyCard = visibleMarkup.match(
			/<div class="f9-wk-empty">([\s\S]*?)<\/div>\s*<section/,
		);
		expect(emptyCard).not.toBeNull();
		expect(emptyCard![1]).not.toContain('href="/capture-rules"');
		expect(emptyCard![1]).not.toContain("Read what we refuse to alert on");
		expect(emptyCard![1]).not.toContain('href="/methodology/ad-aggression-score"');
		expect(emptyCard![1]).not.toContain("How the score works");
		// The empty card's section head carries no doc link either.
		const secHead = visibleMarkup.match(
			/<div class="f9-wk-sec-acts">([\s\S]*?)<\/div>\s*<div/,
		);
		expect(secHead![1]).not.toContain("Read what we refuse to alert on");
		expect(secHead![1]).not.toContain("How the score works");
