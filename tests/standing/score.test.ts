...
describe("standing score", () => {
  it("reads the weight effective on that week and ignores a later one", () => {
    const rows = [
      ...SEEDED,
      { key: "mention_matters", weight: 9, effectiveFrom: "2026-10-01T00:00:00.000Z" },
    ];
  ...
  it("anchors the week on the workspace zone and stores a UTC instant", () => {
    ...
    expect(nextRolloverInstant(new Date("2026-03-09T12:00:00.000Z"), "America/New_York", 1, 8)).toBe(
      "2026-03-16T12:00:00.000Z",
    );
  });

  it("treats a missing week and a gap over 8 days as catch-up, and a 7-day gap as current", () => {
    expect(needsCatchUp(null, WEEK)).toBe(true);
    expect(needsCatchUp("2026-09-12T08:00:00.000Z", WEEK)).toBe(true);
