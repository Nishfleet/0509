...
import { d4PackSchema, hashPack, type d4RequestBody } from "../../workers/jev/context-pack";
import { parseVerdict, postReadThisFirst, selectReadThisFirst, whyLineFor } from "../../workers/standing/read-this-first";
import { quietWeekLine } from "../../workers/standing/present";
...
describe("D4 read this first", () => {
  ...
  it("posts one noul and one score in a single request", async () => {
    const fetchImpl = vi.fn(() => Promise.resolve(new Response(JSON.stringify({
      answers: {
        read_this_first: { type: "noul", noul: 0.72, reason: "Pricing moved" },
        importance: { type: "score", score: 8 },
      },
    }), { status: 200 })));
    const verdict = await postReadThisFirst("https://api.typesafe.ai/v1/systemone", "secret", pack, fetchImpl);
    expect(fetchImpl).toHaveBeenCalledOnce();
    const init = fetchImpl.mock.calls[0]?.[1];
    expect(init?.method).toBe("POST");
    const body = JSON.parse(String(init?.body)) as ReturnType<typeof d4RequestBody>;
    expect(body.model).toBe("jev-latest");
    expect(body.questions.read_this_first.type).toBe("noul");
    expect(body.questions.importance.type).toBe("score");
    expect(body.questions.importance.criteria).toHaveLength(11);
    expect(verdict).toEqual({ p: 0.72, importance: 8, reason: "Pricing moved" });
  });

  it("reads a fleet-proxy probability when noul is absent", () => {
    expect(parseVerdict({
      answers: {
        read_this_first: { type: "boolean", probability: 0.4 },
  ...
  it("hashes the same pack once, independent of key order", async () => {
    ...
    const second = await hashPack(d4PackSchema.parse({
      item: pack.item,
