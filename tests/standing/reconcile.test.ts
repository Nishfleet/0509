...
import { createCatchUp, replaceRollover, type RolloverParams } from "../../workers/standing/schedule";
...
describe("standing reconciliation", () => {
  ...
  it("does not enqueue a second catch-up while one is still waiting", async () => {
    ...
    const store = db();
    const id = await createCatchUp(roll.api, store.api, clock, "standing_ws_1_1", new Date("2026-09-22T03:00:00.000Z"));
    expect(id).toBeNull();
