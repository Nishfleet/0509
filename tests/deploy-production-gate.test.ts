...
`,
  ...
  it("prints sanitized hydration error detail on predeploy readiness failure", () => {
    const root = mkdtempSync(join(tmpdir(), "0509-readiness-diag-hydration-"));
    roots.push(root);
    const manifestPath = join(root, "manifest.json");
    writeFileSync(
      manifestPath,
      JSON.stringify({
        ...
        entries: [
          ...
          { sourceFile: "journal-of-the-other.spec.ts", status: "passed" },
        ],
      }),
    );
    const lines: string[] = [];
    ...
    expect(output).toContain("Journey 3: monitoring loop (mobile)");
    expect(output).toContain("/app/watchlists?watchlist=e2e-1");
    expect(output).not.toContain("journal-of-the-other");
  });
  ...
  it("anchors on the recorded head only when it exists in this history; a rewritten-away head falls through to the bootstrap, loudly (0509#2975)", () => {
    ...
    // bootstrap the helper fails with the regenerate-the-evidence phrase,
    // never a raw git Command failed.
    expect(() =>
    ...
    expect(warnings.some((w) => w.includes(PINNED_EVIDENCE_SHA_NOT_IN_HISTORY))).toBe(true);
    warnings.length = 0;
  ...
  it("fails a rewritten-away pinned SHA with regenerate-the-evidence, not Command failed (0509#2974)", async () => {
    ...
    // Classification must not crash the prepare job (exit 2). An orphaned
    // recorded deploy with no bootstrap is a verdict: strictest evidence
    // policy, so prepare falls through to generate_restore_evidence.
    ...
    expect(
      warnings.some((line) => line.includes(PINNED_EVIDENCE_SHA_NOT_IN_HISTORY)),
    ).toBe(true);
  }, 30_000);
