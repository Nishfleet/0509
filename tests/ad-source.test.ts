...
import {
  ...
  expectCacheNotWritten,
  expectNoProviderCooldown,
  expectProviderStateCooledDown,
  expectProviderStateWritten,
  failingPage,
  jsonPage,
  latestProviderStateWrite,
  mockDiscoveryDataModule,
  runDiscoverySourceContract,
  type FixturePageFetcher,
} from "./helpers/discovery-source-contract";
...
const META_AD_FILTERS: SearchFilters = {
  query: "nykaa",
  country: "India",
  platform: "all",
  creativeType: "all",
...
/**
 * Drives one Meta pagination case: the fixture fetcher becomes the Meta API
 * page fetch, and the entry point the interactive-depth path is reached
 * through is selectable. A case therefore only declares fixtures and
 * expectations.
 */
async function driveMetaSource(input: {
  entry: "interactive" | "resolver";
  fetchPage: FixturePageFetcher;
  extraPages: number;
  query?: string;
}): Promise<SearchResponse> {
  vi.doMock("~/lib/meta-api.server", () => ({
    searchAds: input.fetchPage,
    demoSearch: vi.fn(),
  ...
  vi.doMock("~/lib/meta-library-browser.server", () => ({
    ...
    getInteractiveMetaApiExtraPages: vi.fn().mockReturnValue(input.extraPages),
    CommercialDiscoveryError: class CommercialDiscoveryError extends Error {},
  }));

  const adSource = await import("~/lib/ad-source.server");
  const query: NormalizedSavedQuery = {
    mode: "advertiser",
    filters: { ...META_AD_FILTERS, query: input.query ?? META_AD_FILTERS.query },
  };
  if (input.entry === "interactive") {
    return adSource.searchMetaApiAdsWithInteractiveDepth({} as never, query, null, {
      interactive: true,
    });
  }
  return adSource.searchAdsViaSourceResolver(
    {
      DB: {} as D1Database,
...
/**
 * Interactive Meta pagination honesty, driven through the shared discovery
 * contract harness (tests/helpers/discovery-source-contract.ts): every case
 * below is page fixtures in, expected records and honesty semantics out.
 */
describe("interactive Meta pagination honesty", () => {
  it("labels and preserves a partial result when a bounded later page fails", async () => {
    ...
    await runDiscoverySourceContract({
      name: "partial later page",
      pages: [
        jsonPage("meta-advertiser-page-1", {
          ads: [firstAd],
          nextCursor: "cursor-2",
          source: "meta_api",
        }),
        failingPage("meta-advertiser-page-2", new Error("page 2 unavailable")),
      ],
      expect: {
        records: [firstAd],
        nextCursor: "cursor-2",
        honesty: {
          ...
          summaryIncludes: "results shown are partial",
        },
      },
      drive: ({ fetchPage }) =>
        driveMetaSource({ entry: "interactive", fetchPage, extraPages: 2 }),
    });
  });
  ...
  it("preserves prior provider success when a later Meta page degrades", async () => {
    ...
    const { upsertDiscoveryProviderState, upsertDiscoveryCacheEntry } =
      mockDiscoveryDataModule({
        providerState: discoveryProviderStateFixture({
          provider: "meta_api",
    ...
    await runDiscoverySourceContract({
      name: "prior provider success preserved",
      pages: [
        jsonPage("meta-advertiser-page-1", {
          ads: [firstAd],
          nextCursor: "cursor-2",
      ...
      drive: ({ fetchPage }) =>
        driveMetaSource({ entry: "resolver", fetchPage, extraPages: 2 }),
    });

    expectProviderStateWritten(upsertDiscoveryProviderState, {
      provider: "meta_api",
      status: "degraded",
      failureClass: "provider_unavailable",
      lastSuccessAt: previousSuccess,
      lastFailureAtIsSet: true,
      partial: true,
    });
    expectCacheNotWritten(upsertDiscoveryCacheEntry);
  });

  it("maps unclassified Meta API first-page failures to provider_unavailable", async () => {
    const { upsertDiscoveryProviderState } = mockDiscoveryDataModule();

    const run = await runDiscoverySourceContract({
      name: "first-page failure classification",
      pages: [
        failingPage(
          "meta-advertiser-page-1",
          new Error("upstream opaque failure"),
        ),
      ],
      expect: {
        records: [],
      ...
      drive: ({ fetchPage }) =>
        driveMetaSource({ entry: "resolver", fetchPage, extraPages: 0 }),
    });
    ...
    // PROVIDER_UNAVAILABLE_COOLDOWN_MS in ad-source.server.ts: a first-page
    // failure cools the provider down globally for five minutes.
    expectProviderStateCooledDown(
      latestProviderStateWrite(upsertDiscoveryProviderState),
      5 * 60 * 1000,
      run.startedAt,
    );
  });
  ...
  it("does not globally cool down Meta after a successful first page was partial", async () => {
    ...
    await runDiscoverySourceContract({
      name: "no global cooldown after a partial success",
      pages: [
        jsonPage("meta-advertiser-page-1", {
          ads: [firstAd],
          nextCursor: null,
    ...
    expectNoProviderCooldown(upsertDiscoveryProviderState);
  });
});
