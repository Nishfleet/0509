...
describe("public proof brief date-only capture stamps", () => {
  ...
  it("keeps the trail capture clock on the date-only source value", () => {
    // The route renders trail capturedAt via the same stamp rule; the brief
    // must pass the date-only value through untouched (no invented time).
  ...
  it("still renders full timestamps with their capture time", () => {
    ...
    // Full timestamp: the clock is real precision and stays.
