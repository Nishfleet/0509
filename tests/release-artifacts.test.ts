...
describe("attachReleaseStateArtifacts", () => {
  it("takes the release screenshot without mutating the DOM", async () => {
    ...
    // [contenteditable] element, then restore it. When that lands while
    // React hydration is still in flight (dev release server, module graph
    // still loading), the hydration diff reports an extra style attribute
    // and the strict proof fails on browser_hydration_error:console.
    // "initial" leaves the DOM untouched.
