...
describe("marketing rebuild", () => {
  ...
  it("leads with a real-data proof hook and never with a fabricated sample story", () => {
    ...
    expect(marketingRoute).toContain("See the Meta ads");
    expect(marketingRoute).toContain("any competitor is");
    ...
    expect(marketingRoute).toContain("Then Five to Nine watches the offer behind those ads");
    expect(marketingRoute).toContain("before-and-after screenshot when it changes");
    expect(marketingRoute).toContain("Weekly on Free, every 3 hours on Starter.");
...
describe("proof strip freshness chip", () => {
	...
	it("gates the populated-state chip label on heroProofLive", () => {
		...
		const populatedHead = marketingRoute.indexOf('We saved the proof');
		expect(populatedHead).toBeGreaterThan(-1);
