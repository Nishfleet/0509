...
/**
 * Issue #3156: EDGE_BINDING_LIMITS above mirrors the RL_* binding capacities
 * declared in wrangler.jsonc, wrangler.e2e.jsonc and the integration fixture
 * (tests/integration/wrangler.test.jsonc). That mirror was only
 * comment-pinned, so a capacity changed on one side could silently drift
 * from the enforcing binding on another. This detector fails the moment any
 * of the four copies disagree.
 */
describe("RL_* binding capacity parity (issue #3156)", () => {
  ...
  function declaredBindingCapacities(
    path: string,
  ): Record<string, { limit: number; period: number }> {
    const raw = readFileSync(path, "utf8");
    // Line-comment stripper (quote-count aware), the same proven pattern as
    // tests/fullsite-watch-enablement.test.ts — wrangler configs are JSONC.
    const withoutComments = raw
      .split("\n")
      .map((line) => {
        const commentIndex = line.indexOf("//");
      ...
      .join("\n");
    const parsed = JSON.parse(withoutComments) as {
      bindings?: Array<{ name?: string; simple?: { limit: number; period: number } }>;
      // Today the RL_* entries live under unsafe.bindings in every wrangler
      // config; accept the graduated top-level shape too so the detector
      // survives the field moving out of `unsafe`.
      unsafe?: { bindings?: Array<{ name?: string; simple?: { limit: number; period: number } }> };
    };
    const rlBindings = (parsed.unsafe?.bindings ?? parsed.bindings ?? []).filter(
      (binding) => binding.name?.startsWith("RL_"),
    );
    return Object.fromEntries(
      rlBindings.map((binding) => [
        binding.name as string,
        binding.simple as { limit: number; period: number },
      ]),
    );
  }

  it("keeps EDGE_BINDING_LIMITS in lockstep with every declared RL_* binding", () => {
    for (const path of WRANGLER_CONFIGS) {
      const capacities = declaredBindingCapacities(path);
      expect(
        Object.keys(capacities).sort(),
        `${path} must declare exactly the EDGE_BINDING_LIMITS bindings`,
      ).toEqual(Object.keys(EDGE_BINDING_LIMITS).sort());
      for (const [binding, limit] of Object.entries(EDGE_BINDING_LIMITS)) {
        expect(
          capacities[binding],
          `${path} ${binding} capacity must match the code constant`,
        ).toEqual({ limit, period: 60 });
      }
    }
  });
});
