...
export const HEADERED_CSV_FIXTURE = [
...
export const ROW_OUTCOME_FIXTURE = [
  "northstar-shoes.com",
...
const ACCEPTED_NAME_HEADERS = ["name", "company", "brand", "competitor", "advertiser"];
const ACCEPTED_WEBSITE_HEADERS = ["domain", "website", "url", "site"];
const ACCEPTED_NOTES_HEADERS = ["note", "notes", "description"];
const ACCEPTED_TAG_HEADERS = ["tag", "tags"];
const ACCEPTED_CLIENT_HEADERS = ["client", "account", "customer"];
const ACCEPTED_HEADERS = [
  ...ACCEPTED_NAME_HEADERS,
  ...ACCEPTED_WEBSITE_HEADERS,
  ...ACCEPTED_NOTES_HEADERS,
  ...ACCEPTED_TAG_HEADERS,
  ...ACCEPTED_CLIENT_HEADERS,
];
...
describe("magicbrief-migration fixtures", () => {
  ...
  it("every documented alias header maps to the same field", () => {
    for (const header of ACCEPTED_NAME_HEADERS) {
    ...
    for (const header of ACCEPTED_WEBSITE_HEADERS) {
...
describe("magicbrief-migration rejected-field report", () => {
  it("unsupported MagicBrief columns never reach imported fields and stay in the row's raw text", () => {
    ...
    expect(row?.raw).toContain("2026-01-01..2026-01-31");
...
describe("magicbrief-migration guide alignment", () => {
  ...
  it("the guide embeds the exact tested fixtures verbatim", () => {
    for (const fixture of ALL_FIXTURES) {
      expect(guide, "guide fixture text drifted from the test fixture").toContain(fixture);
    }
  });

  it("the guide states the parser's real limits", () => {
    expect(guide).toContain(`${Math.floor(COMPETITOR_IMPORT_MAX_BYTES / 1024)} KB`);
  ...
  it("the guide states the truthful boundary and the manual fallback", () => {
    expect(guide).toMatch(/no full MagicBrief export contract/i);
    expect(guide).toContain("rejected-field report");
    ...
    expect(guide).toContain("Non-claims");
    expect(guide).toContain("no real MagicBrief export fixture");
  });

  it("the guide's creation-path claims match the code that persists them", () => {
    const actionSource = readFileSync("app/lib/setup-checklist-action.server.ts", "utf8");
    expect(actionSource).toContain("client_reports");
    expect(actionSource).toContain("upsertClientRoom");
    expect(actionSource).toContain("import_context");
    expect(actionSource).toContain("market_desk_import");
    expect(guide).toContain("client_reports");
    expect(guide).toContain("import_context");
    expect(guide).toContain("market_desk_import");
  });
});
