...
describe("public document header", () => {
  it("keeps the legal header brand, subtitle, and focus ring readable on the bone nav", () => {
    expect(css).toContain(`.f9-legal-page .f9-app-brand .f9-wordmark {
  color: var(--ld-ink, #0e0d0a);`);
    expect(css).toContain(`.f9-legal-page .f9-app-brand small {
  color: var(--ld-ink, #0e0d0a);`);
    expect(css).toContain(`.f9-legal-page .f9-app-brand:focus-visible {
...
.f9-legal-page .f9-search-nav-links a:focus-visible { color: var(--ld-ink); }`);
    expect(css).not.toContain(`.f9-legal-page .f9-app-brand .f9-wordmark {
  color: var(--ld-bone, #f4f1e8);`);
    expect(css).not.toContain(`.f9-legal-page .f9-app-brand small {
  color: var(--ld-bone, #f4f1e8);`);
    expect(css).not.toContain(`.f9-legal-page .f9-app-brand:focus-visible {
  outline-color: var(--ld-bone, #f4f1e8);`);
  });

  it("keeps the app-brand override stronger than the later shared legal wordmark rule", () => {
    const appBrandRule = css.indexOf(".f9-legal-page .f9-app-brand .f9-wordmark {");
