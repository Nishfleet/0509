 ...
 *
 * Escape: `// dead-sha: <why>` on or above the literal line — for literals
 * used as opaque strings that never go through git object resolution.
 */
    ...
    for (const file of dirs.flatMap((d) => listTestFiles(d))) {
      ...
      lines.forEach((line, i) => {
        for (const match of line.matchAll(GIT_OBJECT_LITERAL)) {
          ...
          const above = lines[i - 1] ?? "";
          if (line.includes(DEAD_SHA_NOTE) || above.includes(DEAD_SHA_NOTE)) {
            continue;
          }
          violations.push(
