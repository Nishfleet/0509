...
describe("deliverScanTroubleNotice", () => {
  ...
  it("terminalizes a provider exception as provider-unknown after dispatch", async () => {
    const send = vi.fn().mockRejectedValue(new Error("provider connection closed"));
    ...
    expect(updateDeliveryAttemptResult).toHaveBeenCalledWith(
      { EMAIL: { send }, EMAIL_FROM_EMAIL: "alerts@0509.io", APP_ORIGIN: "https://0509.io" },
      "attempt-1",
