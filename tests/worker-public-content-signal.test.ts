...
import { CONTENT_SIGNAL } from "../workers/security-headers";
 ...
 * not re-read per page (workers/app.ts, workers/security-headers.ts).
 *
...
describe("public HTML content-signal through the worker fetch handler (issue #2302)", () => {
  ...
  it("sends content-signal on public deep pages, with the security headers intact", async () => {
    const worker = await loadWorker();
