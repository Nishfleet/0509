...
describe("SignupFirstBriefView waiting state (#3176 progress)", () => {
  beforeEach(() => {
    // The shared helper's contract: reset first, THEN mock, and import the
    // component INSIDE the test — otherwise the component's `Link` binding
    // keeps the real (context-hungry) react-router export.
    vi.resetModules();
    mockReactRouter();
  });
  ...
  async function renderWaiting(data: SignupFirstBriefLoaderData): Promise<string> {
    const { SignupFirstBriefView } = await import(
      "~/components/signup-first-brief-view"
    );
    return renderToStaticMarkup(createElement(SignupFirstBriefView, { data }));
  }

  it("renders the denominator, the finished count, and every per-source tick", async () => {
    const html = await renderWaiting(
      waitingData({
        scanProgress: {
          total: 3,
          done: 1,
  ...
  it("marks every source's status with its state so the words are machine-greppable too", async () => {
    const html = await renderWaiting(
      waitingData({
        watchlistName: null,
        scanProgress: {
  ...
  it("hides the block when the run has not planned its fan-out yet", async () => {
    const html = await renderWaiting(waitingData({}));

    expect(html).not.toContain("f9-signup-scan-progress");
    // renderToStaticMarkup escapes the apostrophe (We&#x27;re).
    expect(html).toContain("scanning Glowkart now.");
  });

  it("an all-zero planning pass hides the block too", async () => {
    const html = await renderWaiting(
      waitingData({
