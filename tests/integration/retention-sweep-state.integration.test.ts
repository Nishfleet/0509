...
describe("retention_sweep_state migration (0085)", () => {
  it("writes and reads the orphan-reconcile cursor row", async () => {
    const stateId = "r2_orphan_reconcile_cursor";
    ...
    await db()
      ...
      .bind(stateId, "first-cursor", "delete", "2026-09-07T12:00:00.000Z")
      .run();

    const read = await db()
      .prepare(
        `SELECT cursor_value, mode FROM retention_sweep_state WHERE state_id = ?`,
      )
      .bind(stateId)
    ...
    await db()
      ...
      .bind(stateId, "second-cursor", "dry-run", "2026-09-07T13:00:00.000Z")
      .run();

    const rows = await db()
      .prepare(`SELECT cursor_value, mode FROM retention_sweep_state WHERE state_id = ?`)
      .bind(stateId)
      .all<{ cursor_value: string | null; mode: string }>();
  ...
  it("allows the cursor to be cleared back to null", async () => {
    ...
    await db()
      ...
      .bind(stateId, "some-cursor", "delete", "2026-09-07T12:00:00.000Z")
      .run();
    ...
    await db()
      ...
      .bind(stateId, null, "delete", "2026-09-07T13:00:00.000Z")
      .run();

    const read = await db()
      .prepare(`SELECT cursor_value FROM retention_sweep_state WHERE state_id = ?`)
      .bind(stateId)
