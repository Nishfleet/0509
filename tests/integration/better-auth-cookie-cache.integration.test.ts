...
describe("Better Auth cookie cache (issue #2979)", () => {
  ...
  it("serves the session from the signed cookie with zero D1 reads, and a plan change lands within one request", async () => {
    ...
    // SECOND consecutive cached request — still zero reads; rules out a
    // one-shot fluke (e.g. an in-memory memo masking the D1 read).
    counted.reset();
    const secondCached = await getBetterAuthSession(
      testAppEnv,
      authedRequest(`${TEST_BASE_URL}/app`, Object.values(cookies)),
    );
    expect(secondCached?.user.id).toBe(userId);
    expect(counted.reads()).toBe(0);
  ...
  it("expires with its TTL and reflects a sign-out from D1", async () => {
    ...
    // the DB session row is deleted. Assert the EXPIRY attribute — a fresh
    // Set-Cookie also contains "name=", so name presence alone proves nothing.
    const signOutResponse = await signOutBetterAuth(testAppEnv, sessionRequest);
    const clearedCookies = headersSetCookie(signOutResponse.headers);
    const clearedToken = clearedCookies.find((cookie) =>
      cookie.startsWith("better-auth.session_token="),
    );
    const clearedData = clearedCookies.find((cookie) =>
      cookie.startsWith("better-auth.session_data="),
    );
    expect(clearedToken).toMatch(/max-age=0/i);
    expect(clearedData).toMatch(/max-age=0/i);
  ...
  it("times the session lookup before (D1 read) vs after (signed cookie)", async () => {
    // Timing probe for issue #2979 (not a gate): measures getBetterAuthSession
    // wall-clock on real workerd + real local D1 — the authenticated-request
    // component of dashboard TTFB that this fix removes from the hot path.
    // Local D1 is in-process, so the delta here UNDERSTATES production, where
    // the read is a cross-region leader round-trip.
    const cookies = await signInWithRealMagicLink();
    const { getBetterAuthSession } = await import("~/lib/better-auth.server");
    const ITERATIONS = 10;

    const timePath = async (jar: string[]) => {
      const samples: number[] = [];
      for (let i = 0; i < ITERATIONS; i += 1) {
        const request = authedRequest(`${TEST_BASE_URL}/app`, jar);
        const t0 = performance.now();
        const session = await getBetterAuthSession(testAppEnv, request);
    ...
    const tokenOnlyMs = await timePath([cookies["better-auth.session_token"]!]);
    const cachedMs = await timePath(Object.values(cookies));
    process.stdout.write(
      `#2979 session-lookup timing (median of ${ITERATIONS}, local D1): ` +
        `token-only ${tokenOnlyMs.toFixed(1)}ms, cookie-cached ${cachedMs.toFixed(1)}ms\n`,
    );
  });
});
