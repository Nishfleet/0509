...
function feedFetcherFor(mode: "feed" | "dead"): typeof fetch {
  return ((input: unknown) => {
    ...
    return feedResponseFor(input);
  }) as typeof fetch;
}

function feedResponseFor(input: unknown): Promise<Response> {
  ...
  // URL-keyed, not call-ordered: the presence robots layer caches the
  // robots.txt per host, so a global call counter can't tell which target a
  // hop belongs to — the (shared) robots hop would eat the seeded failure.
...
async function seedTrackedEntityWithRssTargets(
  ...
  // real rows, not mocks.
  // Target 0 (the returned head) always serves the healthy feed; every later
  // target serves the 500ing one — the caller's [good, dead] destructure stays
  // true regardless of the presence robots cache sharing one host across the
  // targets.
  ...
  for (let index = 0; index < targetCount; index += 1) {
    ...
    const targetUrl =
      index === 0
        ? "https://1.1.1.1/brand-feed.xml"
        : "https://1.1.1.1/brand-feed-dead.xml";
    await db()
      .prepare(
...
describe("signup scan-source progress on real D1 (#3176)", () => {
  ...
  it("the mention leg polls through the real customer poll path and captures into the mention table", async () => {
    ...
    // at the fan-out's own level. The fixture fetcher keys by URL, so the
    // robots hop (shared host, cached by the presence layer) never steals the
    // seeded failure.
    await expect(
