...
describe("brand-page alias resolution (issue #1446)", () => {
  ...
  it("does NOT redirect the alias when its canonical page is not populated (criterion 4: keep weak alias noindex)", async () => {
    ...
    // snapshot whose only ad does NOT link to the brand (a landing page on an
    // unrelated host), so the wall has ZERO verified-linked ads and the page
    // self-noindexes (issue #1442: indexability is a content-thinness rule).
    const mocks = installMocks({
      byDomain: (domain) => {
        ...
        if (domain === "ridge.com") {
          return [
            ad({
              ...
              // Unrelated landing host → not verified-linked → thin → noindex.
