...
describe("hardened GitHub runner services", () => {
  it("uses separate non-login identities and dedicated routing labels", () => {
    for (const instance of ["verify1", "verify2", "verify3", "deploy", "monitor"]) {
    ...
    expect(provisioner).toContain("vps-deploy");
  ...
  it("keeps each unit below the aggregate cap and reserves host headroom", () => {
    ...
    expect(provisioner).toContain("CPUQuota=150%");
    expect(provisioner).toContain("MemoryMax=4G");
