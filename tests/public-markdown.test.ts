...
describe("public markdown", () => {
  ...
  it("keeps agent-readable content aligned with launch truth", () => {
    ...
    expect(PUBLIC_MARKDOWN).toContain("Scout is the entry plan after the public read-only search and sample watch");
    expect(PUBLIC_MARKDOWN).toContain("weekly digest delivery, and 50 saved change records/month");
    expect(PUBLIC_MARKDOWN).toContain("daily and weekly digest delivery, and 250 saved change records/month");
    expect(PUBLIC_MARKDOWN).toContain("Usage bundles add purchased saved change records that never expire");
    expect(PUBLIC_MARKDOWN).toContain("Included saved change records reset monthly and do not roll over");
    expect(PUBLIC_MARKDOWN).toContain("Scheduled monitoring spends saved records");
    ...
    expect(LLMS_TEXT).toContain("Purchased record packs never expire");
    expect(LLMS_TEXT).toContain("included saved records reset monthly without rollover");
