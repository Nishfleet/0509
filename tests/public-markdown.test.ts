...
describe("public markdown", () => {
  ...
  it("keeps agent-readable content aligned with launch truth", () => {
    ...
    expect(PUBLIC_MARKDOWN).toContain("Generous monthly caps reset each cycle");
    expect(PUBLIC_MARKDOWN).toContain("included proof captures do not roll over");
    expect(PUBLIC_MARKDOWN).toContain("Scheduled scans are included with your plan");
    expect(PUBLIC_MARKDOWN).toContain("saved proof-backed captures use proof captures");
    expect(PUBLIC_MARKDOWN).not.toContain("checks/month");
    expect(PUBLIC_MARKDOWN).not.toContain("Check packs add purchased checks");
    ...
    expect(LLMS_TEXT).toContain("saved proof-backed captures use proof captures");
    expect(LLMS_TEXT).toContain("generous rollover caps");
    expect(LLMS_TEXT).not.toContain("checks/month");
    expect(LLMS_TEXT).not.toContain("Purchased checks never expire");
