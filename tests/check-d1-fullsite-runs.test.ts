...
describe("check-d1-fullsite-runs (#2110)", () => {
  ...
  describe("rowsFromWranglerJson + scanCountFromRows", () => {
    ...
    it("maps a missing or malformed count to 0", () => {
      expect(scanCountFromRows([])).toBe(0);
      expect(scanCountFromRows([{ scan_count: "not-a-number" }])).toBe(0);
      expect(scanCountFromRows([{ scan_count: -2 }])).toBe(0);
    });
  });
});
