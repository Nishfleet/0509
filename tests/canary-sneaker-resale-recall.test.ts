...
describe("canary.sneaker-resale-recall", () => {
  ...
  it("evaluateSneakerResaleRecall surfaces (does not fail) known identity-gap brands", () => {
    // goat.com / on.com / reebok.com (issue #1950) and zappos.com (issue
    // #2059) are classified as identity-resolution gaps (major advertisers
    // whose ads the pipeline does not yet connect to their domain). The
    // canary reports them with their tracking issue rather than hard-failing
    // the guard, mirroring search-tier-canary's alias-gap handling.
    const results = loadSneakerResaleDomains().map((entry) => ({
      domain: entry.domain,
      brand: entry.brand,
      status: 200,
