...
describe("search loader", () => {
  ...
  it("renders rate-limit error state with policy copy, Retry-After countdown, and working Try again button", async () => {
    ...
    vi.doMock("~/lib/customer-route-error", async () => {
      const actual = await vi.importActual<typeof import("~/lib/customer-route-error")>(
        "~/lib/customer-route-error",
      );
      return {
        ...actual,
        mapCustomerRouteError: () => ({
          title: "Too many searches",
          message: "You've hit the anonymous search limit. Free search allows 20 searches per 10 minutes — wait a few minutes and try again.",
          retryable: true,
        }),
      };
    });
    ...
    vi.doUnmock("~/lib/customer-route-error");
  });
