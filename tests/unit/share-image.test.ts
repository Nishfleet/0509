...
describe("share card", () => {
  ...
  it("has nothing to share until there is a ranking", () => {
    expect(shareCard({ payload: null, entities: ENTITIES, sources: [], counts: [], schedule: SCHEDULE, now: NOW })).toBeNull();
    expect(card({ headline_rank: null })).toBeNull();
    expect(
      shareCard({ payload: payload(), entities: ENTITIES.slice(0, 1), sources: [], counts: [], schedule: SCHEDULE, now: NOW }),
    ).toBeNull();
  });
});
