...
describe("harvestHeadings", () => {
  ...
  it("reads nothing when a heading is only its number", async () => {
    const names = await harvestHeadings("<h2>1.</h2><h2>2) </h2><h2></h2>", SUBJECT.name);
...
describe("newsGenerator roundup headings", () => {
  ...
  it("also skips a news.google.com subdomain", async () => {
    const candidates = await newsGenerator(
      SUBJECT,
      fetchTextWithArticle("https://x.news.google.com/rss/articles/X"),
    );

    expect(candidates).toEqual([]);
  });

  it("skips the article when the final url is unparseable", async () => {
    const candidates = await newsGenerator(SUBJECT, fetchTextWithArticle("not a url"));

    expect(candidates).toEqual([]);
  });

  it("keeps a host that merely ends with the Google News name", async () => {
    const candidates = await newsGenerator(
      SUBJECT,
    ...
    const alphalete = candidates.find((candidate) => candidate.name === "Alphalete Athletics");
    const evidence = alphalete?.evidence.find((item) => item.sourceUrl.endsWith("/best-leggings"));
    expect(evidence).toBeDefined();
  });
