...
// is migrated.
//
// The timed runs sign in over the real mail path first — one fresh e2e+
// address per input, so each card lands in a fresh workspace — then the 30 s
// budget is measured from the input submit, never from sign-in. The loader's
// streamed response is captured for the transport + browserMsUsed proof.
