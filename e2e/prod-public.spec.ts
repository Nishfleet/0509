...
test.describe("public production-safe E2E smoke", { lock: "external-api" }, () => {
  test("public pages and machine-readable surfaces render without auth", async ({ page, baseURL, request }) => {
    ...
    // no"): the worker-served robots.txt names the training deny list
    // (GPTBot, ClaudeBot, …) itself (issue #3535). The full allow/deny
    // matrix, including the explicit Google-Extended / OAI-SearchBot /
    // PerplexityBot grounding groups, lives in tests/robots-aeo-policy.test.ts
    // (issue #2061). This live smoke only pins signals that stay true after
    // deploy, so the check does not race the worker rollout.
