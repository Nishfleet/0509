...
// This spec traces to the `/`, `/login` and `/privacy` rows in
// .agents/skills/verify/feature-map.md: each route's footer carries the Privacy
// link and the takedown address support@0509.io, in the same order with the
// same markup. The landing cannot import the shared Footer (it is a static
// document), so its footer is a hand-kept copy of the same two anchors.
//
...
for (const route of PUBLIC_ROUTES) {
  test(`${route} footer carries privacy and the takedown address`, async ({ page }) => {
    ...
    // The two anchors sit in the footer's own source order, compared without
    ...
    const [supportIndex, privacyIndex] = await Promise.all([
