...
INSERT INTO workspace_delivery_config (id, user_id, sensitivity_mode, instant_enabled, digest_enabled, email_enabled, whatsapp_enabled, quiet_hours_json, timezone, created_at, updated_at, slack_enabled) VALUES
  ('e2e-delivery-config-starter', 'e2e-starter', 'balanced', 1, 1, 1, 0, '{"start":"22:00","end":"08:00"}', 'Asia/Kolkata', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-14 days'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), 0);
...
INSERT INTO watch_event (id, watchlist_id, run_id, event_type, status, importance_score, ad_id, baseline_from_run_id, candidate_id, proof_capture_id, title, summary, metadata_json, confirmed_at, suppressed_at, invalidated_at, last_evaluated_at, created_at) VALUES
  ('e2e-event-confirmed', 'e2e-watchlist-starter-1', 'e2e-run-starter-1', 'ad_new', 'confirmed', 86, 'e2e-ad-1', NULL, NULL, 'e2e-proof-capture-starter', 'Okara launched a new workflow offer', 'Fixture confirmed proof-backed event.', '{"source":"verified_proof","from":"Workflow offer baseline","to":"New AI workflow launch","recommendedAction":"Review offer positioning","proofTrail":"Cloudflare Browser proof captured"}', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes'), NULL, NULL, strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes')),
  ('e2e-event-agency-confirmed', 'e2e-watchlist-agency-1', 'e2e-run-agency-1', 'ad_new', 'confirmed', 91, 'e2e-ad-1', NULL, NULL, 'e2e-proof-capture-agency', 'Okara launched a new workflow offer', 'Agency fixture confirmed proof-backed event.', '{"source":"verified_proof","from":"Workflow offer baseline","to":"New AI workflow launch","recommendedAction":"Prepare client counter-positioning","proofTrail":"Cloudflare Browser proof captured"}', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes'), NULL, NULL, strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-58 minutes')),
...
INSERT INTO digest_item (id, digest_run_id, watchlist_id, watchlist_name, event_type, title, summary, metadata_json, created_at) VALUES
  ('e2e-digest-item-1', 'e2e-digest-starter-1', 'e2e-watchlist-starter-1', 'Okara competitor watch', 'ad_new', 'Okara launched a new workflow offer', 'Fixture digest item with verified proof.', '{"eventId":"e2e-event-confirmed","proofStatus":"confirmed","priority":"high"}', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-30 minutes'));
