...
/**
 * BL-032 live proof — /app/digests in the landing language.
 *
 * Same harness contract as BL-031's capture, extended to this surface
 * and to two probes the Briefs audit needs:
 *
 ...
 *    per page and the old Briefs surface stacked them through its filters,
 *    date band, section labels, and facts. A source test can
 ...
 *
 * Opt in with `BL032_CAPTURE=1` (optionally `BL032_OUT=<dir>`). It writes the
 ...
 */
const ENABLED = process.env.BL032_CAPTURE === "1";
const OUT_DIR =
  process.env.BL032_OUT ?? "/home/nish/workspaces/products/0509-audit-artifacts-bl032";
const PREFIX = process.env.BL032_PREFIX ?? "after";
 ...
 */
const SURFACES = [
  { name: "brief-starter-list", url: "/app/digests", user: "e2e-starter" },
  {
    name: "brief-starter-reader",
    url: "/app/digests",
    user: "e2e-starter",
    anchor: "#first-brief-detail",
  },
  {
    name: "brief-starter-filtered",
    url: "/app/digests?urgency=low",
    user: "e2e-starter",
    anchor: "#first-brief-detail",
  },
  {
    name: "brief-free-filed",
    url: "/app/digests?firstrun=1",
    user: "e2e-free-firstbrief",
    anchor: "#first-brief-detail",
  },
  { name: "brief-free-empty", url: "/app/digests", user: "e2e-free-onboarded" },
  { name: "brief-scout-empty", url: "/app/digests", user: "e2e-scout" },
  { name: "brief-agency-empty", url: "/app/digests", user: "e2e-agency" },
  { name: "watchlists-coexistence", url: "/app/watchlists", user: "e2e-starter" },
] as const;
 ...
 * hardcoded greens outside the token system, which no class-scoped probe
...
async function auditPaint(page: Page) {
  return page.evaluate(() => {
    ...
    const paintedAnnouncementMarks = announcementMarks.filter((node) =>
    ...
    return {
      ...
      boxes,
      frames,
      announcementMarks: announcementMarks.length,
      paintedAnnouncementMarks: paintedAnnouncementMarks.length,
      paintedAnnouncementValues: paintedAnnouncementMarks.map(
...
async function measure(page: Page) {
  return page.evaluate(() => ({
    ...
    // The page's filled buttons. The budget is one per VIEWPORT, not
    ...
    // The per-element stack above the first record, so the intent
...
test.describe("BL-032 live proof", () => {
  test.skip(!ENABLED, "set BL032_CAPTURE=1 to write the phase evidence set");
  test.setTimeout(20 * 60 * 1000);

  test("captures every Briefs plan/state, the reader, and the scroll sweep", async ({
    browser,
    baseURL,
  }) => {
    const base = baseURL!;
    ...
    for (const surface of SURFACES) {
      ...
      for (const theme of once ? (["light"] as const) : THEMES) {
        for (const viewport of once ? [VIEWPORTS[1]] : VIEWPORTS) {
          ...
          const anchor =
            "anchor" in surface ? (surface as { anchor: string }).anchor : null;
          if (anchor) {
          ...
          // Paint budgets are measured at the requested anchor. Reset only for
          // the full-page image so every evidence file begins at the page top.
          await page.evaluate(() => window.scrollTo(0, 0));
          await page.waitForTimeout(50);
          await page.screenshot({ path: file, fullPage: true });

          const rebuilt = surface.name.startsWith("brief-");
          metrics.push({
            ...
            announcementMarks: paint.announcementMarks,
            paintedAnnouncementMarks: paint.paintedAnnouncementMarks,
            paintedAnnouncementValues: paint.paintedAnnouncementValues,
          ...
          const expectedAnnouncement = paint.announcementMarks > 0 ? 1 : 0;
          if (paint.paintedAnnouncementMarks !== expectedAnnouncement) {
            failures.push(
              `${surface.name} ${viewport.name} ${theme}: ${paint.paintedAnnouncementMarks} painted newest announcements, expected ${expectedAnnouncement} ` +
                `(${paint.announcementMarks} structurally newest mark(s))`,
            );
          }
          ...
          const unexpectedBoxes = paint.boxes.filter(
            (box) => !box.includes(".f9-wk-btn"),
          );
          if (unexpectedBoxes.length > 0) {
            failures.push(
              `${surface.name} ${viewport.name} ${theme}: boxed surfaces — ` +
                unexpectedBoxes.join(", "),
            );
          }
    ...
    expect(
      failures,
      "one green, one paint-real newest announcement, three caps-mono, no boxes beyond the filled action, one filled button PER VIEWPORT, one rule weight, 44px targets, zero console errors, zero horizontal scroll 320-2560",
    ).toEqual([]);
  });
});
