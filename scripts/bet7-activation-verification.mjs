...
/** @typedef {{ absolutePath: string; relativePath: string }} PersistPath */
/** @typedef {{ origin: string; port: number; release: () => Promise<void> }} ReservedOrigin */
/** @typedef {{ userId: string; watchlistId: string; runId: string; adId: string; eventId: string }} SeededIds */
/** @typedef {{ digestRow: Record<string, unknown> | null; deliveryRow: Record<string, unknown> | null }} FirstBriefState */
/** @typedef {{ email: string | null; domain: string; windowMinutes: number; json: boolean; keepServer: boolean; keepPersist: boolean }} CliArgs */
 ...
 *
 * @param {{ token: string; email: string; domain: string; persistPath: string }} input
 * @returns {SeededIds}
 */
...
/**
 * Poll the dev server origin until it answers, or fail after the boot timeout.
 *
 * @param {string} origin
 * @param {ChildProcess} serverProc
...
async function main() {
  ...
  try {
    ...
    serverProc = spawn(
      ...
      { cwd: REPO_ROOT, env: /** @type {NodeJS.ProcessEnv} */ (env) },
    );
    ...
    try {
      ...
      throw new Error(`onboard page failed to load: ${error instanceof Error ? error.message : String(error)}\n${readServerTail()}`);
    }
...
/**
 * Read the first_brief digest_run + any delivery row for a user from the
 * isolated local D1.
 *
 * @param {{ dbPath: string; userId: string }} input
 * @returns {FirstBriefState}
 */
