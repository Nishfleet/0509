...
for (const [brandId, entry] of Object.entries(doc.graph ?? {})) {
  ...
  for (const [peerId, confidence] of Object.entries(entry.peers ?? {})) {
    ...
    const edgeKey = `${brandId} -> ${peerId}`;
    if (seenEdges.has(edgeKey)) {
      errors.push(`${edgeKey} [${entry.category}]: duplicate edge — the PK is (brand_id, peer_brand_id), a second category for the same pair would be silently swallowed by INSERT OR IGNORE`);
    }
