#!/usr/bin/env bash
# Run the vitest suite with a single, signature-gated retry for the runner
# that hosts `codex-node-checks`.
#
...
# lanes can push a single fork worker past that budget, and because the suite
# starts a fresh fork worker per test file (isolate=true, maxWorkers=1), the
# whole Test step dies on one unlucky start. That failure is transient and is
# not a verdict on the code under test.
#
...
# mask a real regression. The retry happens inside the deploy-window lane the
# CI step already acquired, so it does not re-enter the verification queue.
#
# Usage:
#   ci-vitest-run.sh [--] [command...]
#
# With no arguments the suite command defaults to `npm run test` (the CI Test
# step). Tests inject fake commands plus CI_VITEST_LOG to exercise the retry
# decision without running the suite.
set -u
...
main() {
  local command=(npm run test) attempt=1 rc=0
