...
import { parseSearchResponseHtml } from "./bet2-live-verification.mjs";
...
function formatProbeLine(probe) {
  ...
  return `${probe.keyword.padEnd(10)} status=${String(probe.status ?? "ERR")} rows=${String(probe.rowCount).padStart(3)} ${tier}`;
}
...
async function main() {
  ...
  for (const probe of verdict.failures) {
    emitLine(`  - ${probe.keyword} (rows=${probe.rowCount}, status=${String(probe.status ?? "ERR")})`);
  }
  process.exit(1);
}
