...
// created_at values are ISO-8601 strings with a 'T' separator; comparing them
// against datetime('now') (space separator) would mis-order, so the cutoff is
// computed here as an ISO literal — same convention as the app's bound params.
const trailingWeekCutoff = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();

const yieldPerWatchlistQuery = `
...
try {
  ...
  const weeklyRows = runQuery(yieldWeeklyQuery);
  ...
  const perWatchlistRows = runQuery(yieldPerWatchlistQuery);
