 ...
 * affordance (issue #1899 acceptance). A 410 is the proof-gate dark state
 * (seed rows exist but no real capture has landed). A 5xx is a server
 * fault. A 200 without the marker is the empty "not stored yet" shell,
 * which is a regression for a watched demo brand that should have a
 * populated ledger by now.
 *
...
function main() {
  ...
  if (args.http) {
    ...
    // alongside the run. The filename is stable per run so the upload step's
    // glob (`demo-brand-timeline-canary-*.txt`) matches it.
    try {
      const { writeFileSync } = require("node:fs");
      const stamp = checkedAt.replace(/[:.]/g, "");
      writeFileSync(
        `${root}/demo-brand-timeline-canary-${stamp}.txt`,
