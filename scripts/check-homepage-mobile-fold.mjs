...
for (const viewport of VIEWPORTS) {
  ...
  await page.screenshot({
    path: `${OUT}/after-${viewport.width}x${viewport.height}.png`,
    fullPage: false,
  });
  await page.close();
  ...
  const stripKept = metrics.stripHidden === false;
  const ok = inputAbove && ctaAbove && noOverflow && noConsole && stripKept;
  results.push({
    viewport,
    liveWall,
    metrics,
