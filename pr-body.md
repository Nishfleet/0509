...
- Full node project suite (what CI runs):
  `npx vitest run --configLoader runner --project node` → 551 files, 6594 tests passed.
- Affected suites: `ads-brand-page.route.test.ts`, `ads-brand-page.signals.test.ts`,
  `ads-brand-page.render.test.tsx`, `sitemap.server.test.ts`, `search-query.test.ts` → all passed.
