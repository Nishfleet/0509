...
- `docs/design-system-ratchet.json`: regenerated by `node scripts/design-system-ratchet.mjs --update` (never hand-edited) — `raw-hex-color` 258→246, `non-token-border-radius` 158→152, `css-gradient` 22→12.
...
- `funcs: node scripts/design-system-ratchet.mjs --update` → ceilings tightened: raw-hex 258→246, radius 158→152, gradient 22→12.
- `bash ./scripts/ci-vitest-run.sh -- vitest run --configLoader runner --project node` → `Test Files 661 passed / Tests 7871 passed`. (Workers project skipped per fleet memory budget — no `migrations/**` or `tests/integration/**` changed.)
...
run-proof: node script design-system-ratchet.mjs exit 0 (443 remaining); `ci-vitest-run.sh -- project node` 661 files / 7871 tests pass; sgscan clean.
