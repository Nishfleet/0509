...
- [ ] phase 2: create `app/lib/mention-query.server.ts` — pure, synchronous `buildMentionQuery(trackedEntity, source)` where `source: "reddit" | "x"`; reddit returns `{ source, query: { subredditCandidates: [...] }, provenance }`; x returns `{ source, query: { q: "<entity label>" OR "<canonical domain>" }, provenance }`; provenance names the probe that surfaced each candidate. No env, no network. Accept optional pure `identity?: { domainAliases?; siteName? }` enrichment param; never call the probe inside.
- [ ] phase 3: in `app/lib/presence-source-coverage.server.ts` `presenceSourceCoverageForDocs()`, change x and reddit rows' `productionStatus` from `"unavailable"` to `"gated"` (mirror rss wording re rollout env, off by default). No env access in the static table.
