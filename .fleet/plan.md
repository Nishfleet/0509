...
- [ ] phase 1: cohort derivation (pure helper + read-only D1 tier lookup) — `deriveSneakerResaleCohort(seedList, tierByDomain)` in `app/lib/sneaker-resale-cohort.ts` filters seed entries to domains where `verifiedCount + likelyCount >= 1`; `getSneakerResaleTierByDomain(env, domains)` in `app/lib/sneaker-resale-cohort.server.ts` reads only the existing `public_search` cache (no live provider calls, no schema change) and returns `{ domain -> { verified, likely, hasCoverage } }`; a domain with no cache row or `unmatched`-only cache row is `hasCoverage: false`; unit test under `tests/sneaker-resale-cohort.test.ts`.
