...
## Live re-verification (same unit, 2026-09-14T09:45Z, after salvage resume)

- `wrangler d1 execute 0509 --remote`: watchlist=22, cohort=21,
  watchlist_run=26, delivery_attempt lane=customer status=sent=63 —
  identical to the 08:59Z receipts.
- digest_item WHERE created_at >= '2026-09-13': 22 rows, all ad_new.
- `node scripts/canary-cta-detector.mjs --json` -> exit 1:
  activeWatchlistCount=21 (was 19 — the remaining first scans landed),
  ctaEventCount=0. Still a real measurement of a live cohort, honest zero.
- `node scripts/canary-digest-headline-ratio.mjs` -> exit 0: period
  2026-09-14, 20 creative-churn items collapsed, ratio 0.000, verdict ok.
- `systemctl --user list-timers | grep digest-headline-ratio`: armed,
  next 2026-09-14 15:23 IST.
