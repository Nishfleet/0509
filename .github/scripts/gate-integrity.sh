...
# is still scanned exactly as before. (issue #1947)
...
def main():
    ...
    for f in files:
        ...
        # `|| true` in pnpm-lock.yaml, which the .yml/.yaml softener rule below
        # used to flag). Only the current filename is excluded; renames FROM a
        # lockfile are not, keeping the exclusion to the narrowest surface.
        # (issue #1947)
