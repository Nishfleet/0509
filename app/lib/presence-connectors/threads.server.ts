...
async function normalizeThreadsMedia(post: ThreadsMedia): Promise<NormalizedPresenceItem | null> {
  ...
  // Only a normalizable public http(s) URL on a threads.net hostname may
  // become the canonicalUrl (house convention, gdelt precedent) — a
  // Threads mention's canonical must actually be a threads.net post, so
  // anything else is skipped, never stored or later bot-fetched. The
  // canonicalUrl is never fabricated from the request URL.
  const permalinkUrl = permalinkRaw ? normalizePublicHttpUrl(permalinkRaw) : null;
  const onThreadsNet =
    permalinkUrl &&
    (permalinkUrl.hostname === "threads.net" || permalinkUrl.hostname.endsWith(".threads.net"));
  const permalink = permalinkUrl && onThreadsNet ? permalinkUrl.toString() : null;
  if (!permalink) {
    return null;
  }
