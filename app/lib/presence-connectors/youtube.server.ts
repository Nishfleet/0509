...
async function normalizeYouTubeSearchResult(
  ...
  const title =
    typeof result.snippet?.title === "string" && result.snippet.title.trim()
      ...
      : "YouTube video";
  // rss:552 house pattern — publishedAt is `string | null` in the response, so
  // the typeof guard both satisfies strict TS and keeps a missing publishedAt
  // as honest null instead of safeIsoDate's new Date(null) = 1970-01-01 epoch
  // (hn reviewer precedent). A missing publishedAt falls back to observedAt.
  const publishedAt = typeof result.snippet?.publishedAt === "string"
    ? safeIsoDate(result.snippet.publishedAt)
    : null;
  const description =
