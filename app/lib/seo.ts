...
/** Escape text placed inside an XML element so URLs/domains can never inject markup. */
...
/**
 * Render the sitemap document from canonical paths. Static paths keep their
 * declared order; dynamic /ads/:domain paths are appended already-sorted by
 * the sitemap helper.
 */
export function renderSitemapXml(paths: readonly string[]) {
...
/**
 * The STATIC sitemap (the 13 declared paths only). This file is imported by
 * client-reachable routes, so the D1-backed dynamic assembly lives in the
 * server-only helper (app/lib/brand-page-sitemap.server.ts) which renders the
 * dynamic document and falls back to this unchanged static body whenever the
 * dynamic set is unknowable.
 */
export function staticSitemapFile(): PublicSeoFile {
  return {
    body: SITEMAP_XML,
