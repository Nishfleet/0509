...
// /ads/:domain BRAND PAGES — DYNAMIC SITEMAP ENTRIES (see
// app/routes/ads.$domain.tsx). The brand pages are indexable by default
// (PUBLIC_BRAND_PAGES_INDEXABLE unset or "1"; "0" is the emergency noindex
// brake — the sitemap then omits them too). The honest-shell, demo-sourced,
// and stale (>7 days) states always self-noindex, so the sitemap NEVER lists
// "/ads/..." statically. The /ads/:domain entries are computed AT RENDER TIME
// from discovery_cache_entry rows that would render the indexable state
// (public_search route context, non-demo source, real ads present, fetched
// within the 7-day freshness window, and exactly the search-v2 domain page-1
// row the /ads loader would read) — see loadIndexableBrandPageDomains in
// app/lib/brand-page.server.ts. The caller (workers/app.ts, server-only)
// passes those computed paths in; this module stays pure so it can never pull
// a *.server module into the client graph. This is a bounded cache READ only —
// sitemap generation NEVER triggers live discovery. Without qualifying rows
// the sitemap falls back to the static path list below.
function sitemapXmlBody(paths: readonly string[]): string {
...
const STATIC_SITEMAP_XML = sitemapXmlBody([...SITEMAP_PATHS]);
...
export interface PublicSeoFile {
  body: string;
  contentType: string;
  cacheControl: string;
}
...
/**
 * Public SEO files: robots.txt, sitemap.xml, and the legacy social card. The
 * sitemap is assembled from the static path list plus the indexable
 * /ads/:domain paths the caller computed from the discovery cache (see the
 * strategy note above SITEMAP_PATHS); with none supplied it stays static.
 */
export function publicSeoFileForPathname(
  pathname: string,
  brandPagePaths: readonly string[] = [],
): PublicSeoFile | null {
  ...
  if (pathname === "/sitemap.xml") {
    return {
      body:
        brandPagePaths.length === 0
          ? STATIC_SITEMAP_XML
          : sitemapXmlBody([...SITEMAP_PATHS, ...brandPagePaths]),
