...
// app/routes/ads.$domain.tsx. When we do add them to a sitemap:
...
//   3. Keep this a cache read at sitemap-render time (or a cached sitemap
//      regenerated by the warmup cron); never let sitemap generation trigger
//      live discovery.
//
// The dynamic part is built at request time by
// app/lib/brand-page-sitemap.server.ts (bounded cache-only D1 reads) and
// appended via sitemapXmlForPathnames below; this file stays the single
// source of the static 13-path skeleton and the XML shape.
/**
 * XML-escape a string for use inside element text/attribute content. Valid
 * sitemap paths never contain these characters (canonicalUrl strips queries,
 * and /ads/:domain entries pass normalizeBrandPageDomain, whose charset is
 * letters/digits/dots/hyphens), so this is defense in depth that can never
 * alter a legitimate URL.
 */
...
/**
 * Build the full /sitemap.xml body: every SITEMAP_PATHS entry first, then the
 * extra pathnames (e.g. "/ads/<domain>" entries from the dynamic brand-page
 * sitemap) in caller order, deduplicated against the static set and each
 * other. With no extras this is byte-identical to the pre-dynamic sitemap.
 */
export function sitemapXmlForPathnames(extraPathnames: readonly string[] = []): string {
  const seen = new Set<string>();
  const paths: string[] = [];
  for (const path of [...SITEMAP_PATHS, ...extraPathnames]) {
    if (seen.has(path)) {
      continue;
    }
    seen.add(path);
...
${paths.map((path) => `  <url><loc>${escapeXmlText(canonicalUrl(path))}</loc></url>`).join("\n")}
</urlset>
`;
}

const SITEMAP_XML = sitemapXmlForPathnames();
