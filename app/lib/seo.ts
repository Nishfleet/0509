...
// app/routes/ads.$domain.tsx. The DYNAMIC sitemap follows the contract below;
// it is generated in app/lib/brand-page-sitemap.server.ts (cache-only, D1
// read at sitemap-render time, never live discovery):
...
//      The dynamic generator re-derives the EXACT cache key the /ads/:domain
//      loader would read under the current rollout mode and provider (see
//      deriveBrandPageCacheLookup) and applies the loader's own snapshot
//      filter — recency and a plausible landing URL alone are not enough.
//   3. Keep this a cache read at sitemap-render time; never let sitemap
//      generation trigger live discovery. D1 failures degrade to the static
//      sitemap below with HTTP 200.

/** Shared shape of every public SEO file served by the Worker. */
...
/** Escape text for XML output so a URL/domain can never inject markup. */
...
/**
 * Build the sitemap body: every static SITEMAP_PATHS entry first (stable
 * order), then the dynamic extra paths sorted and deduplicated, so output is
 * deterministic and no URL is ever emitted twice.
 */
export function buildSitemapXml(extraPaths: readonly string[]): string {
  ...
  for (const path of [...extraPaths].sort()) {
    const url = canonicalUrl(path);
    if (seen.has(url)) {
      continue;
    }
...
${urls.map((url) => `  <url><loc>${escapeXmlText(url)}</loc></url>`).join("\n")}
</urlset>
`;
}
...
export function publicSeoFileForPathname(pathname: string): PublicSeoFile | null {
  ...
  if (pathname === "/sitemap.xml") {
    // The STATIC sitemap (no /ads/* entries). The Worker serves the dynamic
    // sitemap via `publicSitemapFile` in brand-page-sitemap.server.ts when the
    // environment is available; this static body is the D1-independent
    // fallback that keeps sitemap availability decoupled from DB health.
    return {
      body: SITEMAP_XML,
