...
// app/routes/ads.$domain.tsx. The dynamic entries are generated at
// sitemap-render time by app/lib/brand-page-sitemap.server.ts from
// discovery_cache_entry rows that would render the indexable state
// (public_search route context, non-demo source, ads present, fetched_at
// within the 7-day freshness window, exact loader-key parity), so we never
// sitemap a page that serves noindex or the "haven't checked recently" shell —
// crawl budget should only go to pages with real cached ads. It is a bounded
// cache read only: sitemap generation NEVER triggers live discovery.
//
// The helpers below keep that dynamic path deterministic and markup-safe:
// `staticSitemapUrls` is the unchanged 13-entry base, `renderSitemapXml`
// escapes every URL so no cached domain can inject XML.

/** Escape the five XML text entities so URL/domain output can never inject markup. */
...
/**
 * Render a sitemap urlset from absolute URLs, in the given order. Every URL
 * is XML-escaped; callers must pass deduplicated, deterministic lists.
 */
export function renderSitemapXml(urls: readonly string[]): string {
  const entries = urls
    .map((url) => `  <url><loc>${escapeXmlText(url)}</loc></url>`)
    .join("\n");
...
/** Absolute canonical URLs for the 13 static sitemap paths, in order. */
export function staticSitemapUrls(): string[] {
  return SITEMAP_PATHS.map((path) => canonicalUrl(path));
}

function staticSitemapXml(): string {
  return renderSitemapXml(staticSitemapUrls());
}
...
export function publicSeoFileForPathname(pathname: string) {
  ...
  if (pathname === "/sitemap.xml") {
    // Static fallback only. The Worker replaces this with the dynamic builder
    // (app/lib/brand-page-sitemap.server.ts) for GET/HEAD; that builder also
    // degrades here on any D1 failure, so sitemap availability never depends
    // on D1 health.
    return {
      body: staticSitemapXml(),
