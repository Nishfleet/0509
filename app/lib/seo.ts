 ...
 * is the published list price, not the live Dodo Payments checkout value
...
const PRICING_TIER_OFFERS: Record<PricingPlanSlug, Record<string, unknown>> = {
  scout: pricingProductOfferJsonLd({
    ...
    priceValidUntilNote:
      "Published EUR list price for the Scout monthly plan. Localized preview at checkout may differ by region and SKU.",
  }),
  starter: pricingProductOfferJsonLd({
    ...
    priceValidUntilNote:
      "Published EUR list price for the Starter monthly plan. Localized preview at checkout may differ by region and SKU.",
  }),
  agency: pricingProductOfferJsonLd({
    ...
    priceValidUntilNote:
      "Published EUR list price for the Agency monthly plan. Localized preview at checkout may differ by region and SKU.",
  }),
};
 ...
 * with an `@graph` root (or one block per product). 4 tier Offers + 3
 * pack Offers = 7 total, matching the issue's verify command threshold
