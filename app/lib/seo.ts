 ...
 * root Layout emit.
 */
function sitemapAlternates(splat: string): { hreflang: string; href: string }[] {
...
function buyerSurfaceSitemapCluster(splat: string): SitemapEntry[] {
  const alternates = sitemapAlternates(splat);
  const enPath = buyerSurfaceEnPathForSplat(splat);
  const freq = STATIC_CHANGEFREQ_PRIORITY[enPath] ?? {
    changefreq: "weekly",
    priority: "0.6",
  };
  return [
    { path: enPath, ...freq, alternates },
    ...BUYER_SURFACE_LOCALE_IDS.map((locale) => ({
      path: `/${locale}/${splat}`,
      ...freq,
      alternates,
...
const EN_SITEMAP_ENTRIES_WITH_HREFLANG: readonly SitemapEntry[] =
  SITEMAP_STATIC_ENTRIES.filter(
    ...
    return splat === null ? entry : { ...entry, alternates: sitemapAlternates(splat) };
  });
