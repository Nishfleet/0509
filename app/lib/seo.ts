...
//      live discovery.
// The dynamic builder lives in app/lib/sitemap.server.ts and follows these
// rules; this static list stays as the base set and the no-DB fallback.
export function buildSitemapXml(locations: ReadonlyArray<string>): string {
...
const SITEMAP_XML = buildSitemapXml(SITEMAP_PATHS.map((path) => canonicalUrl(path)));
