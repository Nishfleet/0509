 ...
 * surface points its `og:image` at a dynamic card served under
...
/**
 * Social card URL for a `guide-pages/<slug>` how-to page (issue #3098). Same
...
/**
 * Social card URL for a `brand-pages/:category` landing page (issue #2067).
 * Mirrors `clusterSocialCardUrl`: the card is derived entirely from the
 * category slug, so the route's `meta` just stamps this absolute URL. The
 * card is generated by `app/lib/social-cards.server.ts` and served from
 * `workers/app.ts` under `/social-card/brand/<slug>.png` (rasterized per
 * issue #3104; the legacy `<slug>.svg` path remains as an alias).
 */
...
/**
 * Canonical consolidation for the duplicate compare-pages/* pairs (issue #1481).
 *
...
/**
 * Reciprocal hreflang set for the resale cluster, including self and
 ...
 * resale cluster (fr/es are pre-evidence for the broader
...
export function publicSeoMeta(input: {
   ...
   * `compare-pages/*`, `switch-pages/*`, `/resale`, `/competitor-monitoring`)
   ...
   * except `guide-pages/<slug>` pathnames, which auto-derive a page-specific
  ...
  // every guide-pages/<slug> route — including ones added later — stamps a
 ...
 * - `comparedProductName`: the competitor product a `compare-pages/*` page is
...
export function webPageJsonLd(input: {
  ...
  return {
    ...
    // the public proof ledger is that a parsed JSON-LD block's @id equals that
 ...
 * indexable multi-level pages (e.g. the public proof ledger) so Google can show a
...
/**
 * schema.org Article for a guide-pages/* editorial page (issue #2855). The
...
/**
 * schema.org `Dataset` for the public Offer Timeline (`the public proof ledger`).
 *
...
export const SITEMAP_PATHS = [
  ...
  "/llms-full.txt",
  // Issue #2152: guide-pages/* how-to cluster. One static entry — the guide is
  ...
  // Lowercase canonical: the issue's exact guide-pages/can-ChatGPT-… slug stays
  // registered in routes.ts and 301s here (issue #2955).
  "/compare",
  // compare-pages/visualping, compare-pages/foreplay, and compare-pages/visualping-ad-library
  ...
  // 40+ stores positioning, public market study). The resale cluster's
  // compare surface.
  ...
  // `/ja/help`, `/decompare-pages/panoramata`, `/de/search`, `/de/ads/nike.com`,
  ...
  // indexable. The genuinely translated resale cluster
  // (`/de/resale` etc., listed above) stays in the sitemap because
 ...
 * across byte-identical duplicates. `/ja/resale` and
 * `/pt-br/resale` are covered by their served `/ja/sitemap.xml` /
 * `/pt-br/sitemap.xml`; `/de/resale` by `/de/sitemap.xml`. Since
 ...
 * `the public proof ledger`, `brand-pages/:slug`, canonicalized-away compare losers
 * such as `compare-pages/visualping`) return `undefined` so the sitemap never
...
export function sitemapHreflangAlternates(path: string) {
  ...
  // The genuinely translated resale cluster ships in de/ja/pt-br
  // only — fr/es have no resale page to point at.
 ...
 * The locale sitemaps are the ONLY path by which `/de/resale`,
 * `/ja/resale`, and `/pt-br/resale` become discoverable —
