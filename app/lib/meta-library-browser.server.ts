...
async function searchMetaLibraryViaSessions(
  ...
  try {
    ...
    // Emits ONE sanitized structured line per Browser Rendering scrape attempt
    // so `wrangler tail` can distinguish login-wall / consent overlay / explicit
    // no-results / captcha-checkpoint / redirect / selector drift when live
    // search returns 0 candidates. Logs ONLY host+path (no query string),
    // lengths, booleans, counts, and a content hash — never page/ad body text.
    // Fire-and-forget: never blocks or perturbs discovery (the async sha256 must
    // not sit on the scrape's critical path).
    void emitScrapeDiagV1({
      step: "browser_sessions",
      requestedUrl,
      finalUrl: safeScrapeDiagPageUrl(page),
      pageText: normalizedExtraction.pageText,
      domCardCount: normalizedExtraction.cards.length,
      textCardCount: extractTextCardsFromVisibleText(
        normalizedExtraction.pageText,
      ).length,
      verifiedCount: scrapeDiagUsableCardCount(extractedCards),
    });
...
// TEMPORARY P0 DIAGNOSTIC — remove after capture (see PR).
//
// Sanitized scrape telemetry for the "0 candidates from Browser Rendering in
// ~18s with no error" production incident. Everything below this banner (plus
// the single `emitScrapeDiagV1(...)` call site in searchMetaLibraryViaSessions
// and the `requestedUrl` capture) is one self-contained diagnostic so a later
// `git revert` removes it surgically. It NEVER emits page body text, ad copy,
// cookies, tokens, or any customer data — only host+path (no query string),
// text lengths, boolean flags, card counts, and a sha256 hash of the page text.
// ============================================================================

export interface ScrapeDiagV1 {
  schema: "scrape_diag_v1";
  step: string;
  finalUrlHost: string;
  finalUrlPath: string;
  ...
  flags: {
    ...
    captchaOrCheckpoint: boolean;
    redirected: boolean;
  };
}

/**
 * Cheap "would this card survive verification" count over the RAW extracted
 * cards — mirrors adHasUsableContent's fields without running the full
 * normalize/analysis pipeline, so the diagnostic adds negligible scrape latency.
 */
function scrapeDiagUsableCardCount(cards: ExtractedAdCard[]): number {
  return cards.filter((card) =>
    Boolean(
      card.advertiser?.trim() ||
        card.body?.trim() ||
        card.previewHeadline?.trim() ||
        card.imageUrl?.trim(),
    ),
  ).length;
}

/** Reads the live page URL defensively — the diagnostic must never throw. */
function safeScrapeDiagPageUrl(page: BrowserPage): string {
  try {
...
/** host + pathname only — deliberately drops the query string (never logged). */
function scrapeDiagHostPath(rawUrl: string): { host: string; path: string } {
  try {
...
/**
 * Pure, synchronous flag-detection core for the scrape diagnostic. Takes the
 * already-in-memory page text plus the requested/final URLs and returns the
 * sanitized diagnostic signals. Cheap case-insensitive marker checks only — no
 * network, no DOM, no customer data. Exported for unit tests.
 */
export function detectScrapeDiagV1(
  step: string,
  pageText: string,
  requestedUrl: string,
  finalUrl: string,
  ...
  const requested = scrapeDiagHostPath(requestedUrl);
  const final = scrapeDiagHostPath(finalUrl);

  const loginWall =
    (text.includes("log in") ||
      text.includes("log into facebook") ||
      text.includes("see more from")) &&
    text.includes("facebook");

  const consentOverlay =
    text.includes("allow all cookies") ||
    text.includes("accept all cookies") ||
    text.includes("allow the use of cookies") ||
    text.includes("only allow essential cookies") ||
    (text.includes("we use cookies") && text.includes("cookies"));

  const explicitNoResults =
    text.includes("no ads found") ||
    text.includes("no ads match") ||
    text.includes("no results") ||
    text.includes("couldn't find any ads") ||
    text.includes("could not find any ads") ||
    text.includes("we didn't find any results") ||
    text.includes("there are no ads");

  const captchaOrCheckpoint =
    text.includes("security check") ||
    text.includes("checkpoint") ||
    text.includes("captcha") ||
    text.includes("confirm you're human") ||
    text.includes("confirm you’re human") ||
    text.includes("prove you're not a robot") ||
    text.includes("prove you’re not a robot");

  // Redirect = the final host or path differs from what we requested. A trailing
  // slash difference is not a redirect; compare normalized paths.
  const normalizePath = (value: string) => value.replace(/\/+$/, "") || "/";
  const redirected =
    final.host !== requested.host ||
    normalizePath(final.path) !== normalizePath(requested.path);

  return {
    schema: "scrape_diag_v1",
    step,
    finalUrlHost: final.host,
    finalUrlPath: final.path,
...
/** Stable sha256 hex of the page text (hash only — the text itself is never logged). */
async function scrapeDiagContentHash(pageText: string): Promise<string> {
  try {
    const data = new TextEncoder().encode(pageText ?? "");
...
/**
 * Emits exactly one sanitized `scrape_diag_v1` JSON line per scrape attempt via
 * console.log (visible in `wrangler tail`). Wrapped so the diagnostic can never
 * throw into — or otherwise perturb — the discovery path.
 */
async function emitScrapeDiagV1(input: {
  step: string;
  requestedUrl: string;
  finalUrl: string;
  pageText: string;
  ...
  try {
    ...
    const line = {
      ...diag,
      counts: {
        domCardCount: input.domCardCount,
        textCardCount: input.textCardCount,
        verifiedCount: input.verifiedCount,
      },
      contentSha256: await scrapeDiagContentHash(input.pageText),
    };
    console.log(JSON.stringify(line));
  } catch {
    // Diagnostic must never break commercial discovery.
  }
}
// ============================================================================
// END TEMPORARY P0 DIAGNOSTIC.
