...
function hasCredentials(env: AppEnv, connectorId: PresenceConnectorId): boolean {
  switch (connectorId) {
    ...
    case "rss":
      ...
      return true;
    case "gdelt":
      // GDELT DOC 2.1 is a free public API with no auth — its terms explicitly
      // allow unlimited commercial use with citation. The rollout gate
      // (PRESENCE_GDELT_ROLLOUT) is still required to activate it.
      return true;
    default:
      return false;
  }
