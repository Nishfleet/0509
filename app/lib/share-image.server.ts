...
export function shareDocument(card: ShareCard, origin: string, cssHref: string = stylesheet): string {
  ...
  return `<!doctype html><html lang="en" data-theme="light"><head><meta charset="utf-8"><base href="${base}"><link rel="stylesheet" href="${displayFaces}"><link rel="stylesheet" href="${textFaces}"><link rel="stylesheet" href="${cssHref}"></head><body>${body}</body></html>`;
}
