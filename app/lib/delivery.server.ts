...
export async function deliverWeeklyDigest(env: AppEnv, input: DeliverWeeklyDigestInput) {
  ...
  // Issue #2175 do-step 4: a quiet brief shows one real public-cohort move
  // (stored public offer-timeline corpus; loader shared with #2147) instead
  // of a bare "all quiet". Loaded once, only for heartbeat digests, and a
  // failed load degrades to the classic all-quiet body.
  const digestPublicMove: DigestPublicMove | null =
    isHeartbeat && emailTargets.length > 0
      ? await loadWeeklyPublicMoves(env, { limit: 1 })
          .then((moves) => {
            const move = moves[0];
            return move
              ? {
                  ...
                  changedAt: move.changedAt,
                  url: `${appBaseUrl(env)}${move.path}`,
                }
              : null;
          })
          .catch(() => null)
  ...
  for (const target of emailTargets) {
    attempts.push(
      await deliverDigestToEmailTarget(
        ...
        { forwardUrl: digestForwardUrl, publicMove: digestPublicMove },
      ),
    );
  }
...
async function deliverDigestToEmailTarget(
  ...
  const email = renderDigestEmail(env, {
    ...
    publicMove: briefExtras.publicMove ?? null,
  });
