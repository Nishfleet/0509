 ...
 *
 * `otherItems` is returned for completeness of the shared helper, but the
 * public /ads route surfaces only headlineItems + adChurnSummary today; any
 * future event type that is neither a landing_page_* headline nor ad churn
 * (e.g. website_page_changed) must be explicitly surfaced by the consumer —
 * it is not silently rendered.
 */
