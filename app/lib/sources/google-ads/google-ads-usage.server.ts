 ...
 * `source_snapshot` rows only record successes, so a failed Transparency
 * Center RPC leaves no trace anywhere. This module mirrors the #2181
 ...
 *
 *   google_ads:captures:YYYY-MM-DD  →  {"attempted": n, "failed": m}
 *
 ...
 * wrangler.jsonc yet — #2181's wiring decision) nothing is written and /status
 * omits the measured line entirely (the #2200 rule: missing = no false
 * claim). The 35-day TTL lets stale day counters expire, exactly like the
 * #2181 month counters.
 */

export interface GoogleAdsCaptureCounters {
  attempted: number;
  failed: number;
}

export interface GoogleAdsCaptureStats extends GoogleAdsCaptureCounters {
  /**
...
function dayKey(date: Date): string {
  ...
  return `google_ads:captures:${iso.slice(0, 10)}`;
}

function parseCounters(raw: string | null): GoogleAdsCaptureCounters {
  if (!raw) return { attempted: 0, failed: 0 };
  try {
    const parsed = JSON.parse(raw) as Partial<GoogleAdsCaptureCounters>;
    return {
 ...
 */
export async function recordGoogleAdsCaptureAttempt(
  env: AppEnv,
  ...
  try {
    ...
    const next: GoogleAdsCaptureCounters = {
 ...
 */
export async function getGoogleAdsCaptureStats24h(
  env: AppEnv,
  now: Date = new Date(),
