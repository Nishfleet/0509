...
/**
 * Options narrowing `indexableBrandPagePathsFromRows` to rows the public page
 ...
 * /ads/:domain paths that the brand page would both find and render indexable.
 * Kept separate from the D1 read so the filtering rules are unit-testable
 * without a database.
 */
...
export async function loadIndexableBrandPagePaths(
  ...
  try {
    ...
    return indexableBrandPagePathsFromRows(rows, now, { provider, useDomainV2 });
  } catch (error) {
