...
/**
 * Pure core: derive indexable /brands/:categorySlug sitemap entries from the
 ...
 * (`brandCategoryForDomain` + `categorySlugForLabel`). A curated category
 * with zero indexable brands is omitted, mirroring the route's empty-category
 * 404 guard (issue #1988) so the sitemap never lists a page that 404s.
 *
 * `lastmod` is the newest `lastmod` among the category's brands (the same
 * freshness signal the brand entries carry), `changefreq=weekly`, and
 * `priority=0.6` (same band as the /brands hub's own static entry). Pure and
 * exported so the rule is unit-testable without a database.
 */
