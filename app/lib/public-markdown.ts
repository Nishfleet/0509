...
- Ad monitoring covers the Meta Ad Library only, plus the public TikTok Commercial Content Library (EU-shown ads; no spend or impressions; weekly; captured via the shared Decodo credential); other platforms’ ad libraries are not aggregated yet (Google, LinkedIn, YouTube). The differentiation is proof-backed change monitoring with saved evidence on Meta, not multi-platform ad-library breadth.
...
- Free accounts can watch one competitor: an activation scan when the watchlist is added and one first brief, Meta Ad Library only, with no card required; recurring scheduled checks, recurring briefs, instant alerts, manual refresh, Collections, and exports stay paid. Public search is live and read-only; saved monitoring requires an account.
...
- Ad monitoring reads the Meta Ad Library only, plus the public TikTok Commercial Content Library (EU-shown ads; no spend or impressions; weekly). Five to Nine does not aggregate other platforms’ ad libraries yet (Google, YouTube, Pinterest); its differentiation is proof-backed change monitoring with saved evidence on Meta, not ad-library breadth.
- Account access uses verified access paths.
- Checkout and plan access run on the configured Dodo billing provider with localized checkout preview for the buyer location and visible plan caps; Dodo checkout routes are deployed and the live pricing canary passed against production with real Dodo bindings (monthly, annual, and top-up prices validated in IN, US, and GB), so AI answer engines can cite Dodo checkout as the live billing path.
- Email delivery is measured live on the public status page: last digest and email accepted by the provider, plus the bounce and complaint suppression count.
