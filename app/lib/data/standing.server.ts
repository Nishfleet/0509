...
import {
  ...
  type HomePanel,
  type SourceHealth,
} from "../../../workers/standing/present";
import { currentWeekStart, nextRolloverInstant, shiftWeek } from "../../../workers/standing/score";
...
export async function loadHome(userId: string): Promise<HomePanel> {
  ...
  const [entities, standing, digest, freshness] = await env.DB.batch([
    env.DB.prepare(
      "SELECT id, name, state, state_reason FROM entity WHERE workspace_id = ?",
    ).bind(workspace.id),
    env.DB.prepare(
  ...
  const sources = (freshness as D1Result<{ name: string; canary: number | null; last_fetched_at: string | null }>).results.map((row) => ({
    name: row.name,
...
function rankedHome(
  ...
  const latestRows = rows.filter((row) => row.week_start_at === latest);
  ...
  const whyIsQuiet = payload?.why_source === "counts" || payload?.quiet_week === true;
  const why = degraded && whyIsQuiet ? null : payload?.why_line ?? null;
  const standingRanks = ordered.flatMap((row) => row.rank === null ? [] : [{ entityId: row.entity_id, rank: row.rank }]);
  return {
    phase: "ranked",
    message: null,
    ...
    brands: ordered.map((row) => ({
      ...
      movement: formatMovement(row.movement),
    ...
    why,
    whyIsJev: !degraded && payload?.why_source === "jev",
    updatedSinceBrief: payload ? briefDisagrees(payload.ranks, standingRanks) : false,
    readFirst: degraded ? [] : (payload?.read_this_first ?? []).slice(0, 3),
    freshness: freshnessLines(sources, workspace.timezone),
...
function arrivalLabel(workspace: WorkspaceRow): string {
  const iso = workspace.next_brief_at ?? nextRolloverInstant(
    new Date(),
    workspace.timezone,
    workspace.brief_weekday,
    workspace.brief_hour,
  );
...
function freshnessLines(sources: SourceHealth[], timeZone: string): { name: string; line: string }[] {
  return sources.map((source) => {
    ...
    const line = source.canary === 0 ? `${source.name}: not answering since ${when}` : `${source.name}: last landed ${when}`;
...
interface PayloadRank { entity_id: string; rank: number }
...
interface PayloadPaused { name: string | null; state_reason: string | null }
interface Payload {
  ranks: { entityId: string; rank: number }[];
  why_line: string | null;
  why_source: string | null;
  quiet_week: boolean;
  read_this_first: HomePanel["readFirst"];
...
function readPayload(raw: string | null): Payload | null {
  ...
  try {
    ...
    const ranks = Array.isArray(record.ranks) ? record.ranks.flatMap((row) => {
      if (!row || typeof row !== "object") return [];
      const item = row as PayloadRank;
      if (typeof item.entity_id !== "string" || typeof item.rank !== "number") return [];
      return [{ entityId: item.entity_id, rank: item.rank }];
    }) : [];
    const readFirst = Array.isArray(record.read_this_first) ? record.read_this_first.flatMap((row) => {
      if (!row || typeof row !== "object") return [];
      const item = row as PayloadRead;
      if (typeof item.signal_id !== "string") return [];
      return [{
        signalId: item.signal_id,
        title: item.title ?? item.signal_id,
        evidenceUrl: item.evidence_url,
        reason: item.reason,
      }];
    }) : [];
    const paused = Array.isArray(record.paused) ? record.paused.flatMap((row) => {
      if (!row || typeof row !== "object") return [];
      const item = row as PayloadPaused;
      return [{ name: item.name ?? null, state_reason: item.state_reason ?? null }];
    }) : [];
    return {
      ranks,
      why_line: typeof record.why_line === "string" ? record.why_line : null,
      why_source: typeof record.why_source === "string" ? record.why_source : null,
      quiet_week: record.quiet_week === true,
      read_this_first: readFirst,
      paused,
    };
  } catch {
    return null;
