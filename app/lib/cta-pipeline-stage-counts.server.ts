...
export function ctaPipelineStageCountsFromCounters(
  ...
  const validityPassed = counters.diff.status === "confirmed";
  // The extract stage is recorded (and ctaFunnelStage becomes non-null) only
  // when the DOM extraction actually ran against a snapshot.
