...
const noPricingPreview: LocalPricingPreview = { available: false };

export async function loader({ context, request }: LoaderFunctionArgs) {
  const { getEnv } = await import("~/lib/context.server");
  const { publicCommercialLaunchSummary } = await import("~/lib/commercial-launch-gate.server");
  const { promiseWithTimeout } = await import("~/lib/fetch-timeout.server");
  const env = getEnv(context);

  let pricingPreview = noPricingPreview;
  try {
    // Real published prices are rendered into the server HTML on first paint
    // (the Dodo checkout preview resolves the buyer's country from the
    // request). A slow or failing preview must never hold up the homepage
    // document, so it is timeboxed and degrades to the published USD anchors.
    const { previewDodo0509PlanPrices } = await import("~/lib/dodo-pricing.server");
    pricingPreview = await promiseWithTimeout(
      previewDodo0509PlanPrices({ env, request }),
      4000,
      "marketing pricing preview timed out",
    );
  } catch {
    // Keep the published anchor prices; the client retries via
    // /api/pricing-preview when the pricing section nears the viewport.
  }
...
function priceLabel(
  ...
  return (
    preview?.prices?.[planId]?.[cycle]?.display ||
    `$${PUBLISHED_PLAN_PRICES_USD[planId][cycle]} USD`
  );
}
...
function bundlePriceLabel(preview: LocalPricingPreview | null, bundleId: UsageBundleSlug) {
  return (
    preview?.usageBundles?.[bundleId]?.display ||
    `$${PUBLISHED_BUNDLE_PRICES_USD[bundleId]} USD`
  );
}
...
export default function MarketingRoute() {
  ...
  const annualCycleAvailable =
    saleOpenPricingPlans.length > 0 && saleOpenPricingPlans.some((plan) => annualAvailableFor(plan.slug));
  const annualSavingsValidated =
    saleOpenPricingPlans.length > 0 && saleOpenPricingPlans.every((plan) => annualAvailableFor(plan.slug));
