...
export function brandPageDescription(data: BrandPageLoaderData): string {
  ...
  if (data.verifiedLinkCount === 0) {
    return `See ${totalCount} Meta ${adWord} matching ${data.domain}, from a public Ad Library check ${data.checkedAgo}. Their link to the site is not verified. Get an email when the ads or offers change.`;
  }
  if (totalCount > 0 && data.brandOwnedAdCount === totalCount) {
    return `See ${totalCount} Meta ${adWord} from ${data.brandName} (${data.domain}), from a public Ad Library check ${data.checkedAgo}. Get an email when their ads or offer change.${unverifiedTail}`;
  }
  if (data.brandOwnedAdCount === 0) {
    return `See ${data.verifiedLinkCount} Meta ${linkWord} from other advertisers linking to ${data.domain}, from a public Ad Library check ${data.checkedAgo}. Get an email when the ads or offers change.${unverifiedTail}`;
  }
  return `See ${data.verifiedLinkCount} Meta ${linkWord} linking to ${data.domain} — ${data.brandOwnedAdCount} from ${data.brandName} and ${otherCount} from other advertisers — from a public Ad Library check ${data.checkedAgo}. Get an email when the ads or offers change.${unverifiedTail}`;
}
...
function closerHonestyLine(
  ...
  if (allBrandOwned) {
    return `Ad creatives are ${data.brandName}'s real ads from the public Meta Ad Library${cached}.${tail}${unverifiedNote}`;
  }
  if (noneBrandOwned) {
    return `Ad creatives are real Meta Ad Library ads from other advertisers linking to ${data.domain}${cached}.${tail}${unverifiedNote}`;
  }
  return `Ad creatives are real Meta Ad Library ads linking to ${data.domain}${cached} — ${data.brandOwnedAdCount} run by ${data.brandName} and ${otherCount} by other advertisers.${tail}${unverifiedNote}`;
}
