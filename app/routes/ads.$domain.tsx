 ...
 *
 * Returns null when the page has no cached ads — the FAQ is grounded in
 * on-page content that only exists for a real capture, so a cache-miss shell
 * (which 301-redirects anyway) never ships one.
 */
  ...
  return [
    {
      ...
      answer: `The Ad Aggression Score is a 0–100 number from a public formula at /methodology/ad-aggression-score. It is the sum of four sub-scores, 0–25 each — Velocity (new ads per week), Testing (share of ads with more than one creative variant), Freshness (how recent the creatives are), and Persistence (how long ads stay live). They add up to the score with no hidden weighting, and the score card on this page shows each one. The score only renders once ${brandName} has at least 14 days of watching and at least one ad with a verified link to ${domain}.`,
    },
    {
  ...
  return (
    <>
      ...
      {/* 6. BRAND FAQ — rendered from the same array as the FAQPage JSON-LD
          ...
          labels, the Watch CTA). Hidden on the cache-miss shell, which
          301-redirects and never reaches this component. */}
      {(() => {
        const faq = brandPageFaqEntries(data);
        if (!faq) return null;
        return (
