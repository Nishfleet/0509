...
const LINK_CLASS =
  "flex min-h-11 w-full min-w-0 flex-col items-center justify-center gap-1 px-1 py-2 text-center font-mono text-[0.66rem] leading-[1.3] tracking-[0.06em] whitespace-nowrap uppercase no-underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink min-[860px]:flex-row min-[860px]:justify-start min-[860px]:gap-3 min-[860px]:px-4 min-[860px]:text-left min-[860px]:text-[0.72rem]";

function AppNav() {
  return (
    <nav aria-label="App" className={NAV_CLASS}>
        ...
        {signedInNav.map((item) => (
          <li key={item.href} className="min-w-0">
            <NavLink
              ...
              className={({ isActive }) => `${LINK_CLASS} ${isActive ? "text-ink" : "text-ink-soft"}`}
            >
              {({ isActive }) => (
                <>
                  <span
...
export function SignedInFrame({ children }: { children: ReactNode }) {
  return (
    <div className="min-[860px]:grid min-[860px]:min-h-dvh min-[860px]:grid-cols-[11rem_minmax(0,1fr)]">
      <AppNav />
      <div className="min-w-0 pb-[92px] min-[860px]:pb-0">{children}</div>
    </div>
  );
}
